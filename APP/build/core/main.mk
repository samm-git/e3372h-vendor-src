...
#add for llt
$(info main.mk: EUAP_LLT=$(EUAP_LLT))
ifeq ($(EUAP_LLT),true)
    $(shell rm -rf $(TOP)/vbsllt/result)
endif

#
# Include all of the makefiles in the system
#
subdirs += $(TOP)/device/huawei-mbb/$(TARGET_PRODUCT)/3rdComponent/qualcomm/$(QCM_PLATFORM_VERSION)
subdirs += $(TOP)/device/huawei-mbb/$(TARGET_PRODUCT)/3rdComponent/hisi

# Can't use first-makefiles-under here because
# --mindepth=2 makes the prunes not work.
subdir_makefiles := \
    $(shell build/tools/findleaves.py --prune=$(OUT_DIR) --prune=.repo --prune=.git --prune=vendor --prune=device $(subdirs) Android.mk)
subdir_makefiles += $(TOP)/vendor/huawei-platform/build/Android.mk
ifdef TARGET_BUILD_MODEL
    subdir_makefiles += $(DEVICE_LOCAL_PATH)/$(TARGET_BUILD_MODEL)/product_build.mk
endif
...
# Copy kernel headers to bionic directory.
kernel_headers_dir := $(TOP)/device/huawei-mbb/$(TARGET_PRODUCT)/3rdComponent/qualcomm/$(QCM_PLATFORM_VERSION)/kernel-headers
bionic_headers_dir := $(TOP)/bionic/libc/kernel/common
kernel_files_exist := $(shell cd device/huawei-mbb && \
    if [ -d $(TARGET_PRODUCT) ]; then echo "true"; else echo "false"; fi;)
ifeq ($(kernel_files_exist), true)
    $(info $(shell $(kernel_headers_dir)/copy_kernel_headers.sh $(bionic_headers_dir)))
endif
...
.PHONY: websdkimage
websdkimage: $(INSTALLED_WEBSDKIMAGE_TARGET)

.PHONY: webuiimage
webuiimage: $(INSTALLED_WEBUIIMAGE_TARGET)

.PHONY: initrdimage
initrdimage: $(INSTALLED_INITRDIMAGE_TARGET)
...
droidcore: files \
	...
	$(INSTALLED_WEBSDKIMAGE_TARGET)\
	$(INSTALLED_WEBUIIMAGE_TARGET)\
	$(INSTALLED_INITRDIMAGE_TARGET)\
