...
function lunch()
{
    ...
    export TARGET_BUILD_MODEL=
    export TARGET_BUILD_MODEL_TYPE=
    ...
    local selection=
    local number=$(echo -n $answer | egrep -o "-" | wc -l)
    if [ -z "$answer" ]
    then
    ...
    local number=$(echo -n $selection | egrep -o "-" | wc -l)
    if [ $number -eq 3 ]
    then
        local product=$(echo -n $selection | sed -e "s/-.*$//")
        local model=$(echo -n $selection | cut -d "-" -f2)
        local type=$(echo -n $selection | cut -d "-" -f3)
        local variant=$(echo -n $selection | cut -d "-" -f4)
         
        if [ "${type}" = "llt" ]; then
            type="release"            
            source vbsllt/linux_avatar_64/HLLT_init.sh true
            echo "EUAP_LLT"=$EUAP_LLT
        else
            source vbsllt/linux_avatar_64/HLLT_init.sh false 
        fi
        
        if [ -d device/*/$product/$model ] ; then
            echo $product $model $type $variant
        else
            echo "** Don't have a model for: '$model' **"
            return 1
        fi
    else
        local product=$(echo -n $selection | sed -e "s/-.*$//")
    ...
    if [ $number -eq 3 ]
    then
        export TARGET_BUILD_MODEL=$model
        export TARGET_BUILD_MODEL_TYPE=$type
    fi
    echo
