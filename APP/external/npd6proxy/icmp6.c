...
/* $Id: icmp6.c 75 2011-08-25 08:08:10Z npd6Project $
 * $HeadURL: https://npd6.googlecode.com/svn/trunk/icmp6.c $
 */
 /*********************************************************************
   History:
    date          author        Problem form No.            desc
 2121101411  s00201037  2012/12/25        fortifyÇåÁã
 3082010042  y00188255  2013/08/23        Ïû³ýfortify±¨¾¯
**********************************************************************/
 ...
 *
 */
int open_packet_socket_br0(void)
{
    int sock, err;
    struct sock_fprog fprog;
    struct sockaddr_ll lladdr;
    ...
    laninterfaceIdx = if_nametoindex(NPDAEMON_BR_IF_NAME);
    if (!laninterfaceIdx)
    {
       flog(LOG_ERR, "Could not get ifIndex for laninterfaceIdx %s", NPDAEMON_BR_IF_NAME);
       return 1;
    }
    sock = socket(PF_PACKET, SOCK_RAW, htons(ETH_P_IPV6) );
    if (sock < 0)
    ...
    lladdr.sll_family = AF_PACKET;
    lladdr.sll_protocol = htons(ETH_P_IPV6);
    lladdr.sll_ifindex = laninterfaceIdx;
    lladdr.sll_hatype = 0;
    lladdr.sll_pkttype = 0;
    lladdr.sll_halen = 0;
    err=bind(sock, (struct sockaddr *)&lladdr, sizeof(lladdr));
    if (err < 0)
    {
        close(err);
        close(sock);
        flog(LOG_ERR, "packet socket bind to laninterfaceIdx %d failed: %s", laninterfaceIdx, strerror(errno));
        return (-1);
    }
    flog(LOG_DEBUG2, "packet socket bind to laninterfaceIdx %d OK", laninterfaceIdx);
 ...
 *
 * Inputs:
 *  sockfd is where the data is waiting.
 *      
 * Outputs:
 *  unsigned char *msg
 *      The data.
...
int get_rx(unsigned char *msg, int sockfd, int timeout) 
{
    ...
    t.tv_usec = 0;
    if( select( sockpkt+1, &rfds, NULL, NULL, &t ) < 0 )
    {
        if (errno != EINTR)
...
void if_allmulti(char *ifname, unsigned int state)
{
    ...
    flog(LOG_DEBUG, "Setting MULTICAST if required.");
...
int init_sockets(void)
{
    ...
    /* Raw socket for receiving LAN NSs */
    lansockpkt = open_packet_socket_br0();
    if (lansockpkt < 0)
    {
        flog(LOG_ERR, "open_packet_socket_br0: failed.");
        errcount++;
    }
