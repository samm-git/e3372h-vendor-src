...
/******************************************************************************
修改日期        修改人        问题单号                 简要描述
2013/08/09      s00208871     3080606414         解决因单板系统时间被修改导致超时函数提前返回
2013/08/23      s00208871     3082101267         添加获取日历时间的接口
******************************************************************************/
#include "includes.h"
...
{
/*BEGIN PN: 3080606414  2013/08/09  by shenxiong    Modified */
  struct timespec tv_clock;    
  int res;
  res = clock_gettime(CLOCK_MONOTONIC, &tv_clock);
  if (0 == res)
  {
      t->sec = tv_clock.tv_sec; 
      t->usec = tv_clock.tv_nsec / 1000; /*纳秒转换为微妙 */
  }
  return res;
/*END PN: 3080606414  2013/08/09  by shenxiong    Modified */  
}

/*BEGIN PN: 3082101267  2013/08/23  by shenxiong    Modified */
int os_get_calendar_time(struct os_time *t)
{
	int res;
	struct timeval tv;
...
/*END PN: 3082101267  2013/08/23  by shenxiong    Modified */
