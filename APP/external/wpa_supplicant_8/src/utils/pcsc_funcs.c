...
#define AT_CARDMODE_SIM "^CARDMODE:1"
#define AT_CARDMODE_USIM "^CARDMODE:2"
...
enum card_type get_card_type()
{
	...
	retVal = at_common_cmd(AT_CARDMODE, AT_CARDMODE_MATCH, result);
	if(0 == retVal)
    {
        if(0 == memcmp(AT_CARDMODE_SIM, result, strlen(AT_CARDMODE_SIM) + 1))
        {
            wpa_printf(MSG_ERROR, "get_card_type(): card type(SIM)\n");
            return CARD_SIM;
        }
        else if(0 == memcmp(AT_CARDMODE_USIM, result, strlen(AT_CARDMODE_USIM) + 1))
        {
            wpa_printf(MSG_ERROR, "get_card_type(): card type(USIM)\n");
            return CARD_USIM;
        }
    }
        
    wpa_printf(MSG_ERROR, "get_card_type()retVal=%d, result=%s: no card\n",retVal, result);
    return CARD_NO;
