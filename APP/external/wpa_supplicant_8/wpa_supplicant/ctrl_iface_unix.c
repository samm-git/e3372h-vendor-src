...
/* BEGIN PN: 2063005057,Added  by l00182072, 2012/7/23*/
#define wpa_stacall_cmd_add_len    (20)
#define wpa_stacall_cmd_str_len    (8)
#define wpa_stacall_cmd_add_len1    (20 + 10)
/* END   PN: 2063005057,Added  by l00182072, 2012/7/23*/
					      ...
					      void *sock_ctx)
{
  ...
  /* BEGIN PN: 2063005057,Added  by l00182072, 2012/7/23*/
  char *cmd_buf = NULL;
  char *cmd_buf_blank = NULL;
  unsigned char cmd_id = 0;
  int is_stacall_cmd = 0;
  char *reply_buf = NULL;
  /* 4096 为开源代码中的返回的字符串长度*/
  const int reply_buf_size = 4096 + wpa_stacall_cmd_add_len;
  char reply_buf1[wpa_stacall_cmd_add_len1] = {0};
  size_t ret_len = 0;
  /* END   PN: 2063005057,Added  by l00182072, 2012/7/23*/
	...
	/* BEGIN PN: 2063005057, Modified by l00182072, 2012/7/23*/
  /* 下面的8为"stacall "字符串的长度。参考开源代码规范*/
  if (0 == os_strncmp(buf, "stacall ", 8)) {
      is_stacall_cmd = 1;
      cmd_id = (unsigned char)atoi(buf + 8);
      /* 匹配是否有空格 */
      cmd_buf_blank = strstr(buf + 8," ");
      if (NULL == cmd_buf_blank){
          wpa_printf(MSG_DEBUG, "cmd has not blank:buf=%s",buf);
          cmd_buf = buf;
      } else {
          cmd_buf = cmd_buf_blank + 1;
      }        
  } else {
      is_stacall_cmd = 0;
      cmd_buf = buf;
	...
	if (os_strcmp(cmd_buf, "ATTACH") == 0) {
	...
	} else if (os_strcmp(cmd_buf, "DETACH") == 0) {
		if (wpa_supplicant_ctrl_iface_detach(&priv->ctrl_dst, &from, fromlen))
			reply_len = 1;
		else
			reply_len = 2;
	} else if (os_strncmp(cmd_buf, "LEVEL ", 6) == 0) {
		...
		reply = wpa_supplicant_ctrl_iface_process(wpa_s, cmd_buf,
							  &reply_len);
	}
	...
	if (reply) {
        if (0 == is_stacall_cmd) {
            ...
            reply_buf = os_malloc(reply_buf_size);
        	if (NULL == reply_buf) {
                wpa_printf(MSG_DEBUG, "malloc reply_buf is err");
        		os_free(reply);
        		return ;
        	}
            os_memset(reply_buf, 0, reply_buf_size);
    
            ret_len = (size_t)os_snprintf(reply_buf, reply_buf_size - 1,
                                    "stacall %d %s", (int)cmd_id, (char*)reply);
            
            sendto(sock, reply_buf, ret_len + 1, 
	    ...
	    if (0 == is_stacall_cmd) {
	        ...
	        ret_len = (size_t)os_snprintf(reply_buf1, wpa_stacall_cmd_add_len1 - 1,
                                    "stacall %d FAIL\n", (int)cmd_id);
            
	        sendto(sock, reply_buf1, ret_len + 1,
	...
	/* END   PN: 2063005057, Modified by l00182072, 2012/7/23*/
					   ...
					   size_t len)
{
	...
	io[idx].iov_len = 0;//os_strlen(levelstr);
	idx++;
