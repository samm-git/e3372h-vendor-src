...
/******************************************************************************
修改日期        修改人        问题单号                简要描述
2012/07/04      l00182072     2041301134        合入v3r2上报错误原因码的处理
...
2014/01/06      s00208871     3122707692        输入错误密码，概率没有提示重新输入密码的对话框
2014/01/07      s00208871     4010206112        输入正确密码进行连接，长时间连接后，没有连接成功，弹出密码框要求重新输入密码
2013/01/11      s00208871     4010807630        Wi-Fi的自動OFF虽然设定到了30分、但是公衆無線LAN的接続持续12分～15分左右就切断了
...
/* BEGIN PN: 2041301134,Added  by l00182072, 2012/7/4*/
extern WPA_4WAY_HANDSHAKE_FLAG is_wpa_4way_handshake_failed;
/* END   PN: 2041301134,Added  by l00182072, 2012/7/4*/
/* BEGIN PN: 2112401041,Added  by l00182072, 2012/11/27*/
extern WPA_SUPPLICANT_PRIVATE_STATE gWpaSupplicantCurrentState;
/* END   PN: 2112401041,Added  by l00182072, 2012/11/27*/
/* BEGIN PN: 3102303890,Added  by s00208871 , 2013/10/23*/
extern WPA_SUPPLICANT_PRIVATE_STATE g_wpa_last_state;
/* END PN: 3102303890,Added  by s00208871, 2013/10/23*/
extern int g_disassoc_flag;
...
/* 博通芯片会上报连接状态，如果收到连接失败，结束认证，重新发起扫描 */
void asso_event_match(void *ctx, char *custom)
{
  struct wpa_supplicant *wpa_s = ctx;

  wpa_printf(MSG_DEBUG, "current state:%s", wpa_supplicant_state_txt(wpa_s->wpa_state));
    
  if (WPA_ASSOCIATING == wpa_s->wpa_state) {
    if((0 == os_strncmp(custom, "Conn NoNetworks", 15)) ||
       (0 == os_strncmp(custom, "Conn ConfigMismatch", 19)) ||
       (0 == os_strncmp(custom, "Conn AuthTimeout", 16)) ||
       (0 == os_strncmp(custom, "Conn AuthNoAck", 14)) ||
       (0 == os_strncmp(custom, "AuthTimeout", 11)) ||
       (0 == os_strncmp(custom, "AuthFail", 8)) ||
       (0 == os_strncmp(custom, "AuthNoAck", 9))) 
    {
        eloop_cancel_timeout(wpa_supplicant_timeout, wpa_s, NULL);
        wpa_printf(MSG_DEBUG, "stop authentication timer and exec it at once");
        wpa_supplicant_timeout(wpa_s, NULL);
    }
  }
}
			      ...
			      enum wpa_states state)
{
  ...
  /* BEGIN PN: 2041301134,Added  by l00182072, 2012/7/4*/
  if(WPA_4WAY_HANDSHAKE == wpa_s->wpa_state && 
      WPA_DISCONNECTED == state) {
  	is_wpa_4way_handshake_failed = WPA_4WAY_HANDSHAKE_FLAG_FAILED;
  	wpa_printf(MSG_DEBUG, "State: set 4way_handshake_failed_flag");
  }
  /* END   PN: 2041301134,Added  by l00182072, 2012/7/4*/
if (state != WPA_SCANNING)
    ...
    /* BEGIN PN: 3102303890, Added  by s00208871 , 2013/10/23*/
    g_wpa_last_state = gWpaSupplicantCurrentState;
    wpa_printf(MSG_DEBUG, "connect complete,record current state:%d\n", g_wpa_last_state);
    /* END PN: 3102303890, Added  by s00208871, 2013/10/23*/
    /*BEGIN 3100300345   s00208871   2013-10-08    Modified*/
    gWpaSupplicantCurrentState = WPA_SUPPLICANT_PRV_CONNECTED;
    wpa_printf(MSG_DEBUG, "set current state to connected");
...
int wpa_supplicant_reload_configuration(struct wpa_supplicant *wpa_s)
{
	...
	/* BEGIN PN: 2111300780, Deleted  by l00182072, 2012/11/9*/
	//eapol_sm_invalidate_cached_session(wpa_s->eapol);
	/* END   PN: 2111300780, Deleted  by l00182072, 2012/11/9*/
...
/* BEGIN PN: 2082707533,Added  by l00182072, 2012/9/3*/
  ...
  if(NULL != bss) {
    if (bss_wpa && bss_rsn) {
        g_wps_auth_type = (WPS_AUTH_WPAPSK | WPS_AUTH_WPA2PSK);
    } else if (bss_rsn) {
        g_wps_auth_type = WPS_AUTH_WPA2PSK;
    } else if (bss_wpa) {
        g_wps_auth_type = WPS_AUTH_WPAPSK;
    } else {
        g_wps_auth_type = WPS_AUTH_OPEN;
    }
    wpa_printf(MSG_DEBUG, "WPA: g_wps_auth_type:%d ",g_wps_auth_type);
  }
#endif
			      ...
			      struct wpa_bss *bss, struct wpa_ssid *ssid)
{
    ...
    g_disassoc_flag = 0; 
    wpa_printf(MSG_INFO, "reset disassoc:%d", g_disassoc_flag);
...
#ifdef CONFIG_WPS    
  g_wps_auth_type = WPS_AUTH_OPEN;
  wpa_printf(MSG_DEBUG, "init g_wps_auth_type to:%d ",g_wps_auth_type);
#endif
  /* END   PN: 2082707533,Added  by l00182072, 2012/9/3*/
...
#ifdef CONFIG_WPS        
    g_wps_auth_type = WPS_AUTH_OPEN;
    wpa_printf(MSG_DEBUG, "NO-WPA: g_wps_auth_type:%d ",g_wps_auth_type);
#endif        
    /* END   PN: 2082707533,Added  by l00182072, 2012/9/3*/
				   ...
				   int reason_code)
{
	...
	if (addr) {
		...
		if (is_zero_ether_addr(wpa_s->bssid) &&
		  wpa_s->wpa_state == WPA_ASSOCIATING) {
  		wpa_printf(MSG_DEBUG, "do not send death event to app");
    } else {
				    ...
				    struct wpa_ssid *ssid)
{
	...
	if (ssid == NULL) {
  	...
  	/* BEGIN PN: 2063002727,Modified by l00182072, 2012/7/3*/
  	if (ssid == wpa_s->current_ssid) {
      wpa_printf(MSG_DEBUG, "disable network");
      wpa_supplicant_cancel_auth_timeout(wpa_s);
      //wpa_supplicant_disassociate(wpa_s, WLAN_REASON_DEAUTH_LEAVING);
  	}
  	/* END   PN: 2063002727,Modified by l00182072, 2012/7/3*/
				   ...
				   struct wpa_ssid *ssid)
{
  ...
  wpa_s->scan_flag = BROADCAST_SSID_SCAN_FLAG;
...
#else 
    	wpa_printf(MSG_ERROR, "Do not listen on bridge:%s", wpa_s->bridge_ifname);
#endif   
	}
...
void wpas_connection_failed(struct wpa_supplicant *wpa_s, const u8 *bssid)
{
	...
	wpa_dbg(wpa_s, MSG_DEBUG, "Blacklist count %d --> request scan in %d "
		"ms(do not triger scan here)", count, timeout);
