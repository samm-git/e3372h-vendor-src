...
u8 g_acwps_wpa_psk[WPS_PBC_WPA_PSK_KEY] = {0};
				   ...
				   const struct wps_credential *cred)
{
    ...
    /* BEGIN PN: 2082707533, Modified by l00182072, 2012/9/3*/
    wpa_printf(MSG_DEBUG, "WPS: auth_alg=%d, key_mgmt=%d, proto=%d",
				   ssid->auth_alg, ssid->key_mgmt, ssid->proto);    
    os_memset(g_acwps_wpa_psk, 0, WPS_PBC_WPA_PSK_KEY - 1);
    
	if (ssid->key_mgmt == WPA_KEY_MGMT_PSK) {
		if (cred->key_len == 2 * PMK_LEN) {
            os_memcpy(g_acwps_wpa_psk, cred->key, cred->key_len);
...
void wpas_wps_notify_scan_results(struct wpa_supplicant *wpa_s)
{
    ...
    /*  修改log输出级别， 从MSG_INFO修改为MSG_DEBUG */
		wpa_msg_ctrl(wpa_s, MSG_DEBUG, WPS_EVENT_AP_AVAILABLE);
	}
}
