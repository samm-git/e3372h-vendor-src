...
2014/06/21      s00208871     4062100027         隐藏SSID连接失败
******************************************************************************/
...
static void wpa_supplicant_scan(void *eloop_ctx, void *timeout_ctx)
{
	...
	if (wpa_s->wpa_state == WPA_INTERFACE_DISABLED) {
		wpa_dbg(wpa_s, MSG_DEBUG, "Skip scan - interface disabled, continue scan");
	...
	if (wpa_s->prev_scan_ssid != WILDCARD_SSID_SCAN) {
		while (ssid) {
			wpa_printf(MSG_INFO, "ssid:%s", ssid->ssid);
	...
	wpa_printf(MSG_INFO, "last_scan_req:%d, ap_scan=%d, max ssids:%d", 
    wpa_s->last_scan_req, wpa_s->conf->ap_scan, max_ssids);
	...
	if (wpa_s->last_scan_req != MANUAL_SCAN_REQ &&
		...
		if (ssid == NULL && max_ssids > 1)
			ssid = wpa_s->conf->ssid;
		while (ssid) {
			/* 去掉对network block是否是disable的判断，默认除了要连接的network外，其它都是disable状态 */
			if (ssid->scan_ssid) {
				wpa_printf(MSG_INFO, "ssid:%s, len:%d", ssid->ssid, ssid->ssid_len);
