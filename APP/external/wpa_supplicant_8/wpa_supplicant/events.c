...
extern unsigned int g_disconnect_reason;
extern WPA_SUPPLICANT_PRIVATE_STATE gWpaSupplicantCurrentState;
extern int g_ScanResultsTimes;
extern void asso_event_match(void *ctx, char *custom);

/*用于在连接过程中，如果扫描不到目标AP，supplicant最大尝试次数 */
#define    MAX_CONN_SCAN_TIMES     10


/* BEGIN PN: 2041301134,Added by l00182072, 2012/7/4*/
WPA_4WAY_HANDSHAKE_FLAG is_wpa_4way_handshake_failed = WPA_4WAY_HANDSHAKE_FLAG_SUCCESS;
/* END   PN: 2041301134,Added by l00182072, 2012/7/4*/

/* BEGIN PN: 2081607635,Added  by l00182072, 2012/9/5*/
extern int g_channel_filter;
/* END   PN: 2081607635,Added  by l00182072, 2012/9/5*/
					      ...
					      int own_request)
{
    ...
    /* 复用scan_flag标识，用于判断是否是app触发的scan动作*/
    if (DEFAULT_SSID_SCAN_FLAG == wpa_s->scan_flag) {
        wpa_printf(MSG_DEBUG, "no scan have been triggered, neglect results");
    } else {		
	    wpa_msg_ctrl(wpa_s, MSG_INFO, WPA_EVENT_SCAN_RESULTS);
    }
	/* EDD PN: 3061809061 ,Added by s00208871, 2013/7/4 */
					      ...
					      int new_scan)
{
    ...
    if (WPA_SUPPLICANT_PRV_CONNECTED == gWpaSupplicantCurrentState) {
        wpa_printf(MSG_DEBUG, "connected state, do not send event to app(%d)", g_ScanResultsTimes);
    }
    if(WPA_SUPPLICANT_PRV_CONNECTING == gWpaSupplicantCurrentState )
    {
        g_ScanResultsTimes = g_ScanResultsTimes + 1;
        wpa_printf(MSG_DEBUG, "scan %d times for connecting", g_ScanResultsTimes);
        if (g_ScanResultsTimes >= MAX_CONN_SCAN_TIMES)
        {
            gWpaSupplicantCurrentState = WPA_SUPPLICANT_PRV_DISCONNECTED;
            wpa_msg(wpa_s, MSG_INFO, WPA_EVENT_DISCONNECTED "100 - Disconnect event - %d times key or SSID mismatch", g_ScanResultsTimes);
					  ...
					  int locally_generated)
{
	...
	if (WPA_SUPPLICANT_PRV_WPSPBCING == gWpaSupplicantCurrentState){
		wpa_printf(MSG_DEBUG, "WPS PBC state and do not report disconnet event");
		return;
	}
	if (!is_zero_ether_addr(bssid) ||
    wpa_s->wpa_state >= WPA_AUTHENTICATING) {
    if(wpa_s->wpa_state == WPA_4WAY_HANDSHAKE && wpa_key_mgmt_wpa_psk(wpa_s->key_mgmt)) {
	    wpa_msg(wpa_s, MSG_INFO, WPA_EVENT_DISCONNECTED "%d" "- Disconnect event - "
      		    "remove keys",WLAN_REASON_4WAY_HANDSHAKE_TIMEOUT);
    } else {
      if( ((1 == g_disassoc_flag) && (WPA_ASSOCIATING == wpa_s->wpa_state)) || 1 == locally_generated ) {
          wpa_printf(MSG_INFO, "do not send disconnect event(disassoc flag:%d, local generated flag:%d)",
            g_disassoc_flag, locally_generated);
      } else {
						 ...
						 int locally_generated)
{
	...
	last_ssid = wpa_s->current_ssid;
 	...
 	/* BEGIN PN: 2062804063, Deleted  by l00182072, 2012/7/10*/
	wpa_supplicant_mark_disassoc(wpa_s);
	/* END   PN: 2062804063, Deleted  by l00182072, 2012/7/10*/
	...
	case EVENT_ASSOC_REJECT:
		...
		if (wpa_s->drv_flags & WPA_DRIVER_FLAGS_SME)
			...
			if (bssid == NULL || is_zero_ether_addr(bssid))
  			...
  			wpa_supplicant_mark_disassoc(wpa_s); 
  			*/
  			/*将reject翻译为authtimeout,重新尝试连接 */
  			asso_event_match(wpa_s, "AuthTimeout");
		}
		break;
