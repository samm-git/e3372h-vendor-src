...
/******************************************************************************
修改日期        修改人        问题单号                 简要描述
2012/07/03      l00182072     2063002727         连接被disable后，删除连接超时函数
2012/07/07      l00182072     2070307199         添加下发最大扫描个数的接口
2012/08/25      l00182072     2082005716         在event_disassoc中添加上报断开
2012/09/26      l00182072     2082707533         STA WPS连接成功后，下次自动连接成功
2012/09/26      l00182072     2063005057         添加序号解决supplicant消息乱序问题  
2012/11/09      l00182072     2111300780         支持EAP-SIM特性
2012/11/26      l00182072     2112006806         解决WPS PBC问题
2013/05/28      s00208871     3052902934         合入指定信道扫描功能，参见问题单3032100903
2013/07/12      s00208871     3061809061         根据scan提供的信道集合对扫描结果进行过滤
2013/10/08      s00208871     3100300345         断开次数统计不准确
2013/10/23      s00208871     3102303890         station wps时，上报对端的加密认证信息
2013/11/20      s00208871     3111807435         wps成功后，上报信道信息
2014/03/29      s00208871     4012203040         添加多个profile时，手动添加一个隐藏SSID，概率出现无法连接成功，成功后高概率扫描不到已经连接的AP
******************************************************************************/
...
/* BEGIN PN: 2070307199,Added  by l00182072, 2012/7/7*/
unsigned int g_scan_maxnum = CSCAN_MAXNUM;
/* END   PN: 2070307199,Added  by l00182072, 2012/7/7*/

/* BEGIN PN: 2082707533,Added  by l00182072, 2012/9/3*/
#ifdef CONFIG_WPS
unsigned int g_wps_auth_type = 0;
int g_wpa_freq = 0;
#endif
/* END   PN: 2082707533,Added  by l00182072, 2012/9/3*/

//w00176398 add wpa_supplicant private status
WPA_SUPPLICANT_PRIVATE_STATE gWpaSupplicantCurrentState =  WPA_SUPPLICANT_PRV_DISCONNECTED;
int g_ScanResultsTimes = 0;

/* BEGIN PN: 3102303890,Added  by s00208871 , 2013/10/23*/
WPA_SUPPLICANT_PRIVATE_STATE g_wpa_last_state = WPA_SUPPLICANT_PRV_DISCONNECTED;
/* END PN: 3102303890,Added  by s00208871, 2013/10/23*/
...
static int set_disallow_aps(struct wpa_supplicant *wpa_s, char *val)
{
	...
	/*
	 * disallow_list ::= <ssid_spec> | <bssid_spec> | <disallow_list> | 摂
...
#ifdef CONFIG_WPS            
      /* BEGIN PN: 3102303890,Added  by s00208871 , 2013/10/23*/
      if (WPA_SUPPLICANT_PRV_WPSPBCING == g_wpa_last_state) {
      /* END   PN: 3102303890,Added  by s00208871 , 2013/10/23*/
        wpa_printf(MSG_DEBUG, "report auth type");
        if ((WPS_AUTH_WPAPSK | WPS_AUTH_WPA2PSK) == g_wps_auth_type) {
              ret = os_snprintf(pos, end - pos, "lw_auth_type=%s\n",
  					  "WPA-WPA2-PSK");
  			  if (ret < 0 || ret >= end - pos)
  				  return pos - buf;
  			  pos += ret;
        } else if (WPS_AUTH_WPAPSK == g_wps_auth_type) {
          ret = os_snprintf(pos, end - pos, "lw_auth_type=%s\n",
  					  "WPA-PSK");
  			  if (ret < 0 || ret >= end - pos)
  				  return pos - buf;
  			  pos += ret;
        } else if (WPS_AUTH_WPA2PSK == g_wps_auth_type) {
          ret = os_snprintf(pos, end - pos, "lw_auth_type=%s\n",
  					  "WPA2-PSK");
  			  if (ret < 0 || ret >= end - pos)
  				  return pos - buf;
        ...
        if (strlen((const char*)g_acwps_wpa_psk)) {
          ret = os_snprintf(pos, end - pos, "lw_psk=%s\n",
            g_acwps_wpa_psk);
  			  if (ret < 0 || ret >= end - pos)
  				  return pos - buf;
  			  pos += ret;
        ...
        /* 添加信道信息 */
        if (0 != g_wpa_freq ) {
          ret = os_snprintf(pos, end -pos, "lw_freq=%d\n", 
            g_wpa_freq);
          if (ret < 0 || ret > end -pos)
            return pos - buf;
          pos += ret;
...
#ifdef ANDROID
  /* 修改log输出级别, 从MSG_INFO修改为MSG_DEBUG */
	wpa_msg_ctrl(wpa_s, MSG_DEBUG, WPA_EVENT_STATE_CHANGE
	...
	const struct wpa_bss *bss, char *buf, size_t buflen)
{
  ...
  if (NULL != ie) {
    ...
    if (ie[1] != 0 && 0 == *(ie + 2)) {
    	size_t ssid_len;
     	u8 *ssid = NULL;
      /* 如果当前处于连接状态，根据bssid进行匹配 */
      if (WPA_COMPLETED == wpa_s->wpa_state && (0 == os_memcmp(wpa_s->bssid, bss->bssid, ETH_ALEN))) {
        if(NULL != wpa_s->current_ssid){
          ssid_len = wpa_s->current_ssid->ssid_len;
          ssid = wpa_s->current_ssid->ssid;
          if (ie[1] == ssid_len && NULL != ssid) {
            wpa_printf(MSG_DEBUG, "get ssid from connected and set '%s' to IE, len:%d", ssid, ssid_len);
            os_memcpy((char*)ie + 2, ssid, ssid_len);
          }
        }
      }
    }
...
/*BEGIN PN: 3052902934  2013/05/24  by shenxiong    ADD */
void get_scan_info(struct wpa_supplicant *wpa_s, char *para)
{

    wpa_printf(MSG_INFO, "%s enter.\n", __FUNCTION__);
    sscanf(para, "%d %llu %llu", &wpa_s->scan_flag, &wpa_s->channels.ch_set[BAND_24G], &wpa_s->channels.ch_set[BAND_5G]);
    if ((0 == wpa_s->channels.ch_set[BAND_24G]) && (0 == wpa_s->channels.ch_set[BAND_5G])) {
        wpa_s->channels.status = FALSE;    
    }
    else {
        wpa_s->channels.status = TRUE;   
    }
    wpa_printf(MSG_INFO, "wpa_s->scan_flag = %d", wpa_s->scan_flag);
    wpa_printf(MSG_INFO, "wpa_s->channels.ch_set_2G = 0x%llx ", wpa_s->channels.ch_set[BAND_24G]);
    wpa_printf(MSG_INFO, "wpa_s->channels.ch_set_5G = 0x%llx ", wpa_s->channels.ch_set[BAND_5G]);
    wpa_printf(MSG_INFO, "%s exit.\n", __FUNCTION__);
}
/*END PN: 3052902934  2013/05/24  by shenxiong    ADD */
/* BEGIN PN: 3061809061 ,Added by s00208871, 2013/7/4 */
extern void dump_scan_channel(struct scan_channel *channels);
/* END PN: 3052902934 ,Added by s00208871, 2013/7/4 */
					 ...
					 char *buf, size_t *resp_len)
{
	...
	if (os_strcmp(buf, "PING") == 0) {
		...
		/* BEGIN PN: 2112006806, Modified by l00182072, 2012/11/26*/
	  gWpaSupplicantCurrentState = WPA_SUPPLICANT_PRV_WPSPBCING;
    ...
    gWpaSupplicantCurrentState = WPA_SUPPLICANT_PRV_WPSPBCING;
    /* END   PN: 2082005716, Modified by l00182072, 2012/8/21*/
    ...
    gWpaSupplicantCurrentState = WPA_SUPPLICANT_PRV_WPSPBCING;
		reply_len = wpa_supplicant_ctrl_iface_wps_pin(wpa_s, buf + 8,
							      reply,
							      reply_size);
    /* END   PN: 2112006806, Modified by l00182072, 2012/11/26*/
	} else if (os_strncmp(buf, "WPS_CHECK_PIN ", 14) == 0) {
		reply_len = wpa_supplicant_ctrl_iface_wps_check_pin(
			wpa_s, buf + 14, reply, reply_size);
	} else if (os_strcmp(buf, "WPS_CANCEL") == 0) {
    /* BEGIN PN: 2082005716, Modified by l00182072, 2012/8/21*/
    gWpaSupplicantCurrentState = WPA_SUPPLICANT_PRV_DISCONNECTED;
	...
	} else if (os_strncmp(buf, WPA_CTRL_RSP, os_strlen(WPA_CTRL_RSP)) == 0)
	{
		...
		wpa_supplicant_deauthenticate(wpa_s,
					      WLAN_REASON_DEAUTH_LEAVING);
	} else if ((os_strncmp(buf, "SCAN ",5) == 0) || (os_strcmp(buf, "SCAN")) == 0) {
    /* BEGIN PN: 3061809061 ,Added by s00208871, 2013/7/4 */
    wpa_printf(MSG_DEBUG, "scan parameter: %s\n", buf);
    wpa_s->scan_flag = DEFAULT_SSID_SCAN_FLAG;
    wpa_s->scan_req = 2;
    if(os_strlen(buf) > 5) {
        get_scan_info(wpa_s, buf + 5);
        dump_scan_channel(&wpa_s->channels);
    }
    /* 如果是广播或者是广播+指定，修改为广播+指定,因为要支持隐藏ssid的扫描 */
    if ((BROADCAST_SSID_SCAN_FLAG == wpa_s->scan_flag) || 
        (MULTI_UNICAST_BROADCAST_SSID_SCAN_FLAG == wpa_s->scan_flag)) {
        wpa_s->scan_flag = MULTI_UNICAST_BROADCAST_SSID_SCAN_FLAG;
    } else {
        wpa_s->scan_flag = MULTI_UNICAST_SSID_SCAN_FLAG;
    }
    wpa_printf(MSG_ERROR, "set scan flag to %d\n", wpa_s->scan_flag);
    ...
    gWpaSupplicantCurrentState = WPA_SUPPLICANT_PRV_CONNECTING;
    /*BEGIN 3100300345   s00208871   2013-10-08    Modified*/
    wpa_printf(MSG_DEBUG, "current scan times:%d\n", g_ScanResultsTimes);
    g_ScanResultsTimes = 0;
    /*END 3100300345   s00208871   2013-10-08    Modified*/
	...
	/* BEGIN PN: 2070307199,Added  by l00182072, 2012/7/7*/
	} else if (os_strncmp(buf, "SCAN_MAXNUM ", 12) == 0) {
	    if(os_strlen(buf) > 12) {
            g_scan_maxnum = atoi(buf + 12);
            wpa_printf(MSG_DEBUG, "g_scan_maxnum:%d",g_scan_maxnum);
            if ((CSCAN_MAXNUM < g_scan_maxnum) || 
                (CSCAN_MINNUM > g_scan_maxnum)) {
                wpa_printf(MSG_DEBUG, "input scan num is err:%d",g_scan_maxnum);
                g_scan_maxnum = CSCAN_MAXNUM;
                reply_len = -1;
            }
	    } else {
	        wpa_printf(MSG_DEBUG, "SCAN_MAXNUM cmd format err");
            reply_len = -1;
	    }
  	/* END   PN: 2070307199,Added  by l00182072, 2012/7/7*/
    }
