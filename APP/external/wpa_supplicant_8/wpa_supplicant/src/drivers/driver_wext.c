...
2013/10/22      s00208871     3102300631         调整assoc prefer的执行位置，从stacall模块，移至wpa_supplicant.解决offload连接成功后，配参数时fw异常
2013/11/20      s00208871     3111807435         记录wps成功后使用的信道值
2014/01/07      s00208871     4010206112         输入正确密码进行连接，长时间连接后，没有连接成功，弹出密码框要求重新输入密码
2014/03/17      s00208871     4031708408         根据代码review对扫描进行优化
2014/03/29      s00208871     4012203040         添加多个profile时，手动添加一个隐藏SSID，概率出现无法连接成功，成功后高概率扫描不到已经连接的AP
2014/05/29      y00187730     4052705641         博通89335芯片不使用信道过滤及combo scan的功能
2014/09/22      y84014385     4082110507         单板已经连接上行AP成功，再次通过WEBUI手动连接AP时，需要页面上点击两次才能连接成功
******************************************************************************/
...
unsigned int g_disconnect_reason = 1;
/* BEGIN PN: 2070307199,Added  by l00182072, 2012/7/7*/
extern unsigned int g_scan_maxnum;
/* END   PN: 2070307199,Added  by l00182072, 2012/7/7*/
#ifdef CONFIG_WPS
extern int g_wpa_freq;
#endif

int g_disassoc_flag = 0;
	...
	while (pos + IW_EV_LCP_LEN <= end) {
		...
		case SIOCGIWAP:
			...
			if (is_zero_ether_addr(
				...
				drv->assoc_resp_ies = NULL;
				g_disassoc_flag = 1;
				wpa_printf(MSG_INFO, "receivd disconnect event and set disassoc flag");
 ...
 */
struct wpa_ssid * next_ssid(struct wpa_ssid *head, struct wpa_ssid *last, int scan_flag)
{
  struct wpa_ssid *p = NULL;
  p = head;
  
  /* 如果是指定ssid扫描，或者上次记录的ssid为空，返回头结点*/
  if(MULTI_UNICAST_SSID_SCAN_FLAG == scan_flag || NULL == last) 
  {
    return p;
  }
  while(p){
    if(p == last){
      wpa_printf(MSG_DEBUG, "find ssid:%s", p->ssid);
      return p->next;
    } else {
      p = p->next;
    }
  }
  wpa_printf(MSG_DEBUG, "fail to find ssid, return head");
  return head;
}

int wpa_driver_wext_combo_scan(void *priv, struct wpa_driver_scan_params *params)
{
	char buf[WEXT_CSCAN_BUF_LEN] = {0};
	struct wpa_driver_wext_data *drv = priv;
	struct iwreq iwr;
	int ret = 0, timeout, i = 0, bp;
  static struct wpa_ssid *last_ssid = NULL;
	struct wpa_ssid *ssid, *ssid_tmp;
	size_t ssid_len = 0;
  struct wpa_supplicant *wpa_s = (struct wpa_supplicant *)(drv->ctx);
	...
	/* Set list of SSIDs */
  ssid_tmp = last_ssid;
  ssid = next_ssid(wpa_s->conf->ssid, last_ssid, wpa_s->scan_flag);

  /* 添加扫描参数的头*/
	os_memcpy(buf, WEXT_CSCAN_HEADER, WEXT_CSCAN_HEADER_SIZE);
  bp = WEXT_CSCAN_HEADER_SIZE;
  /* 如果是广播+指定,增加广播字段 */
  if (MULTI_UNICAST_BROADCAST_SSID_SCAN_FLAG == wpa_s->scan_flag) {
      os_memcpy(buf+bp, WEXT_CSCAN_BROADCAST, WEXT_CSCAN_BROADCAST_SIZE);
      bp += WEXT_CSCAN_BROADCAST_SIZE;
      wpa_printf(MSG_DEBUG, "For scan: broadcast");
  }
	/* BEGIN PN: 2070307199,Added  by l00182072, 2012/7/7*/
  wpa_printf(MSG_DEBUG, "wext_combo_scan: g_scan_maxnum:%d", g_scan_maxnum);
	while (i < (int)g_scan_maxnum) {
	/* END   PN: 2070307199,Added  by l00182072, 2012/7/7*/
		...
		if (ssid == NULL)
			break;
		if ((MULTI_UNICAST_SSID_SCAN_FLAG == wpa_s->scan_flag 
  ...
  /* 扫描时不指定信道，根据当前国家码对应的信道扫描 */
  wpa_s->channels.status = FALSE;
  /*BEGIN 3052902934 shenxiong 2013-05-28 added*/
  bp += wext_set_chan_list(buf + bp, &wpa_s->channels);
  /*END 3052902934 shenxiong 2013-05-28 added*/
	if (MULTI_UNICAST_SSID_SCAN_FLAG == wpa_s->scan_flag||
    MULTI_UNICAST_BROADCAST_SSID_SCAN_FLAG == wpa_s->scan_flag) {
		/* Set numbers of probe (default is 3) */
		buf[bp++] = WEXT_CSCAN_NPROBE_SECTION;
		buf[bp++] = (u8)1;

		/* Set active dwell time (default is 250) */
		buf[bp++] = WEXT_CSCAN_ACTV_DWELL_SECTION;
		buf[bp++] = (u8)40;
		buf[bp++] = (u8)(0 >> 8);
...
#ifdef FEATURE_MBB_WIFI_VSDB
		buf[bp++] = (u8)60;
		wpa_printf(MSG_INFO,"home dwell:%d", 60);
#else
		buf[bp++] = (u8)40;
#endif
		buf[bp++] = (u8)(0 >> 8);
    wpa_printf(MSG_INFO, "user unicast scan parameter");
	}	else {
  	/* Set passive dwell time (default is 250) */
  	buf[bp++] = WEXT_CSCAN_PASV_DWELL_SECTION;
    ...
    wpa_printf(MSG_INFO, "user broadcast scan parameter");
	}
	os_memset(&iwr, 0, sizeof(iwr));
...
static struct scan_channel * chan_set = NULL;
...
#if !defined(FEATURE_HUAWEI_MBB_BCM89335)
  /* BEGIN PN: 3061809061 ,Added by s00208871, 2013/7/4*/
  if (wpa_freq_is_filter(chan_set, data->res.freq)) {
      wpa_printf(MSG_DEBUG, "bssid " MACSTR " ssid ='%s' freq=%d is filtered",
        MAC2STR(data->res.bssid), data->ssid, data->res.freq);
      return;
  }
  /* END PN: 3061809061 ,Added by s00208871, 2013/7/4*/
#endif
...
struct wpa_scan_results * wpa_driver_wext_get_scan_results(void *priv)
{
  ...
  /* BEGIN PN: 3061809061 ,Added by s00208871, 2013/07/4*/ 
  struct wpa_supplicant *wpa_s = (struct wpa_supplicant *)(drv->ctx);
  if(NULL != wpa_s) {
      chan_set = &wpa_s->channels;
  }
  /* END PN: 3061809061 ,Added by s00208871, 2013/7/4*/
...
/*BEGIN 3102300631 s00208871 2013-10-26 added*/
#ifdef FEATURE_MBB_WIFI_VSDB
#define   ASSOC_PREF_B  "/system/bin/wifi_brcm/exe/wl -i WiFi0 assoc_pref b"
#define   ASSOC_PREF_A  "/system/bin/wifi_brcm/exe/wl -i WiFi0 assoc_pref a"
#endif
/*END 3102300631 s00208871 2013-10-26 added*/
			      ...
			      struct wpa_driver_associate_params *params)
{
	...
	if (params->freq && wpa_driver_wext_set_freq(drv, params->freq) < 0)
		ret = -1;
  g_wpa_freq = params->freq;
  wpa_printf(MSG_DEBUG, "record freq:%d", g_wpa_freq);   
  /*BEGIN 3102300631 s00208871 2013-10-26 added*/
#ifdef FEATURE_MBB_WIFI_VSDB
  wpa_printf(MSG_DEBUG, "current assoc freq:%d", params->freq);
  if(params->freq <= FREQ_24G_14) {
    wpa_printf(MSG_DEBUG, "assoc prefer b");      
    system(ASSOC_PREF_B);
  } else {
    wpa_printf(MSG_DEBUG, "assoc prefer a");      
    system(ASSOC_PREF_A);
  }
#endif  
  /*END 3102300631 s00208871 2013-10-26 added*/
...
static int wpa_driver_priv_driver_cmd( void *priv, char *cmd, char *buf, size_t buf_len )
{
    ...
    else if(os_strcasecmp(cmd, "FLUX") == 0){
        os_strncpy(cmd, "FLUX", MAX_DRV_CMD_SIZE);
    }
    else if(os_strcasecmp(cmd, "NO_FLUX") == 0){
        os_strncpy(cmd, "NO_FLUX 100", MAX_DRV_CMD_SIZE);
        wpa_printf(MSG_DEBUG, "%s cmd %s buf_len:%d", __func__, cmd, buf_len);
