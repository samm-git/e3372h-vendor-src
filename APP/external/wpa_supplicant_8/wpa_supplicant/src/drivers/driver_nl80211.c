...
/******************************************************************************
修改日期        修改人        问题单号                 简要描述
2014/06/13      s00208871     4061301303         添加下发最大扫描个数的接口
2014/07/01      s00208871     4063006905         根据下发的扫描参数进行过滤
******************************************************************************/
#include "includes.h"
#include <sys/ioctl.h>
#include <sys/types.h>
					 ...
					 size_t buf_len)
{
	...
	if (os_strncasecmp(cmd, "RSSI", 4) == 0) {
		nl80211_get_link_signal(drv, &si);
		ret = os_snprintf(buf, buf_len, "rssi %d",  si.current_signal);
...
static int bss_info_handler(struct nl_msg *msg, void *arg)
{
	...
	if (wpa_freq_is_filter(chan_set, r->freq)) {
		wpa_printf(MSG_DEBUG, "bssid " MACSTR " freq=%d is skiped",
