...
#define EAP_DOMAIN_NAME_MAX_LEN  48
...
static void * eap_sim_init(struct eap_sm *sm)
{
	...
	wpa_printf(MSG_DEBUG, "EAP-SIM: min_num_chal=%d",data->min_num_chal);
	wpa_printf(MSG_DEBUG, "EAP-SIM: result_ind=%d",data->result_ind);
...
/* BEGIN PN: 2122500375,Added  by l00182072, 2012/11/9*/
#ifdef  SUPP_EAP_SIM_TEST
#define SUPP_EAP_AUTH_ID_NAME "/var/run/eap_id_name.conf"
int eap_sim_set_next_ids(u8 *pseudonym, size_t pseudonym_len,
                    u8 *reauth, size_t reauth_len)
{
    FILE *fp = NULL;

    wpa_printf(MSG_DEBUG, "EAP-SIM: eap_sim_set_next_ids in");
    
    fp = fopen(SUPP_EAP_AUTH_ID_NAME, "w");
    if (NULL == fp) 
    {
        wpa_printf(MSG_DEBUG, "EAP-SIM: open eap file err");
        return -1;
    }

    fwrite("pseudonym: ", strlen("pseudonym: "), 1, fp);
    if (pseudonym)
    {        
        fwrite(pseudonym, pseudonym_len, 1, fp);
        wpa_printf(MSG_DEBUG, "EAP-SIM: next pseudonym=%s "
				   "pseudonym_len=%d", pseudonym, pseudonym_len);
    }
    fwrite("\n", strlen("\n"), 1, fp);

    fwrite("reauth: ", strlen("reauth: "), 1, fp);
    if (reauth)
    {        
        fwrite(reauth, reauth_len, 1, fp);
        wpa_printf(MSG_INFO, "EAP-SIM: reauth_id=%s "
				   "reauth_id_len=%d", reauth, reauth_len);
    }
    fwrite("\n", strlen("\n"), 1, fp);
...
#endif
/* END   PN: 2122500375,Added  by l00182072, 2012/11/9*/
...
/* BEGIN PN: 2122500375,Added  by l00182072, 2012/11/9*/
#ifdef  SUPP_EAP_SIM_TEST
    eap_sim_set_next_ids(data->pseudonym, data->pseudonym_len,
...
/* BEGIN PN: 2111300780, Modified by l00182072, 2012/11/9*/
    ...
    int ret = 0;
#endif
    wpa_printf(MSG_DEBUG, "EAP-SIM: eap_sim_learn_ids in");
	if (attr->next_pseudonym) {
        ...
        os_memset(data->pseudonym, 0,
            attr->next_pseudonym_len + EAP_DOMAIN_NAME_MAX_LEN + 1);
...
#ifdef PCSC_FUNCS        
        ret = eap_sm_add_domain_name(data->pseudonym, &(data->pseudonym_len));
        if (0 != ret) {
            wpa_printf(MSG_INFO, "EAP-SIM: Failed to add domain name in learn ids");
        }
#endif
	}
	...
	if (attr->next_reauth_id) {
        ...
        os_memset(data->reauth_id, 0, attr->next_reauth_id_len + 1);
						 ...
						 struct eap_sim_attrs *attr)
{
	...
	eap_sim_clear_identities(sm, data,  CLEAR_PSEUDONYM | CLEAR_REAUTH_ID | CLEAR_EAP_ID);
...
static Boolean eap_sim_has_reauth_data(struct eap_sm *sm, void *priv)
{
    ...
    /* BEGIN PN: 2111300780,Added  by l00182072, 2012/11/9*/
    wpa_printf(MSG_DEBUG, "EAP-SIM: enter eap_sim_has_reauth_data：");
    if (data->pseudonym) {
        wpa_printf(MSG_DEBUG, "EAP-SIM: has pseudonym data");
    }
    if (data->reauth_id) {
        wpa_printf(MSG_DEBUG, "EAP-SIM: has reauth_id data");
    }
    /* END   PN: 2111300780,Added  by l00182072, 2012/11/9*/
