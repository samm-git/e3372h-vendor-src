...
2012/12/25      l00182072     2122500375          增加打印log的调试宏和相关代码
2014/02/21      s00208871     4022104555          添加eap鉴权失败上报原因码
******************************************************************************/
#include "includes.h"
...
/* BEGIN PN: 2112006806,Added  by l00182072, 2012/11/26*/
extern WPA_SUPPLICANT_PRIVATE_STATE gWpaSupplicantCurrentState;
/* END   PN: 2112006806,Added  by l00182072, 2012/11/26*/
...
SM_STATE(EAP, FAILURE)
{
  ...
  wpa_printf(MSG_DEBUG, "EAP: State=%d ",gWpaSupplicantCurrentState);
  if (WPA_SUPPLICANT_PRV_WPSPBCING == gWpaSupplicantCurrentState)
  {
      wpa_printf(MSG_DEBUG, "EAP: wps not report disconnect ");
  }
  else
  {   /*EAP 鉴权失败后，复用四步握手失败的原因码，上层归一处理 */
      wpa_msg(sm->msg_ctx, MSG_INFO, WPA_EVENT_DISCONNECTED
          "%d EAP authentication disconnected", WLAN_REASON_4WAY_HANDSHAKE_TIMEOUT);
      gWpaSupplicantCurrentState = WPA_SUPPLICANT_PRV_DISCONNECTED;
  }
#ifdef SUPP_EAP_SIM_TEST
  eap_sim_set_next_ids(NULL, 0, NULL, 0);
#endif    
...
#ifdef PCSC_FUNCS
#define IDENTITY_FIRST_SECTION_POSTFIX "@wlan.mnc"
#define IDENTITY_SENCOND_SECTION_POSTFIX ".mcc"
#define IDENTITY_THIRD_SECTION_POSTFIX ".3gppnetwork.org"
#define EAP_AUTH_DOMAIN_NAME "/var/run/eap_domain.conf"
#define DOMAIN_MATCH "domain_name="
#define DOMAIN_NAME_MAX_LEN 48
#define EAP_IMSI_NAME_MAX_LEN 100
#define EAP_DOMAIN_NAME_MAX_BUF 256
#define EAP_IMSI_DOMAIN_MAX_LEN 100

/*解析文件中域名的函数 */
static char * eap_domain_get_line(char *s, int size, FILE *stream, int *line,
				  char **_pos)
{
	char *pos = NULL;
    char *end = NULL;
...
/*增加域名的函数 */
int eap_sm_add_domain_name(u8 *identity, size_t *identity_len)
{
    FILE *f = NULL;
	char buf[EAP_DOMAIN_NAME_MAX_BUF] = {'\0'};
	int line = 0;
	char *pos = NULL;
	int tokenLen = 0;
    int len = 0;
	char domain_name[DOMAIN_NAME_MAX_LEN] = {'\0'};
	size_t mnc_len = 0;  
    const size_t mcc_len = 3; /* mcc的长度*/ 
    const size_t mnc_2_digit = 2; /* mnc的长度为2的值*/
    const size_t mnc_3_digit = 3; /* mnc的长度为3的值*/
    char imsi[EAP_IMSI_NAME_MAX_LEN] = {0};
    size_t imsi_len = 0;

    if ((NULL == identity) || (NULL == identity_len))
    {
        wpa_printf(MSG_ERROR, "identity ptr is null\n");
        return -1;
    }
    
	f = fopen(EAP_AUTH_DOMAIN_NAME,"rb");

	if(NULL == f)
	{
		wpa_printf(MSG_ERROR, "open file %s failed\n", EAP_AUTH_DOMAIN_NAME);
	} 
	else 
	{
	
		while(eap_domain_get_line(buf, sizeof(buf), f, &line, &pos))
		{
			wpa_printf(MSG_ERROR, "line in");
			if(0 == strncmp(pos,DOMAIN_MATCH,sizeof(DOMAIN_MATCH) - 1))
			{
				len = strlen(pos);
				tokenLen = sizeof(DOMAIN_MATCH) - 1;
				if(DOMAIN_NAME_MAX_LEN > (len - tokenLen))
				{
					memcpy(domain_name, pos + tokenLen, (len - tokenLen));
				}
				break;
			}
		}
		wpa_printf(MSG_ERROR, "domain_name ok");
	...
	if('\0' == domain_name[0])
	{
        ...
        if (wpa_scard_get_imsi(imsi, &imsi_len)) 
        {
        	wpa_printf(MSG_WARNING, "Failed to get IMSI from SIM");
        	return -1;
        }
    
        strncat((char*)identity, IDENTITY_FIRST_SECTION_POSTFIX, strlen(IDENTITY_FIRST_SECTION_POSTFIX));

        mnc_len = scard_get_mnc_len(NULL);
        if(mnc_2_digit == mnc_len)
        {
            strncat((char*)identity, "0", 1);
            strncat((char*)identity, imsi + mcc_len, mnc_2_digit);
        }
        else
        {
            strncat((char*)identity, imsi + mcc_len, mnc_3_digit);
        }

        strncat((char*)identity, IDENTITY_SENCOND_SECTION_POSTFIX, strlen(IDENTITY_SENCOND_SECTION_POSTFIX));
        strncat((char*)identity, imsi, mcc_len);
        strncat((char*)identity, IDENTITY_THIRD_SECTION_POSTFIX, strlen(IDENTITY_THIRD_SECTION_POSTFIX));
       	*identity_len = strlen((char*)identity);
	}
	else
	{
		strcat((char*)identity, domain_name);
		*identity_len = strlen((char*)identity);
	}
    /* BEGIN PN: 2122500375, Modified by l00182072, 2012/12/25*/
#ifdef  SUPP_EAP_SIM_TEST    
	wpa_printf(MSG_ERROR, "identity:%s,identity_len=%d \n ",identity, *identity_len);
#endif    
    /* END   PN: 2122500375, Modified by l00182072, 2012/12/25*/
    return  0;
}
/*
				...
				struct eap_peer_config *conf)
{
	...
	//s00208871 原来申请的内存太少，使用中会越界
	conf->identity = os_malloc(EAP_IMSI_DOMAIN_MAX_LEN);
    ...
    memset(conf->identity, 0, EAP_IMSI_DOMAIN_MAX_LEN);
	conf->identity[0] = aka ? '0' : '1';
	os_memcpy(conf->identity + 1, imsi, imsi_len);
	conf->identity_len = 1 + imsi_len;
    /*w00176398 add for EAP-SIM/AKA imsi postfix*/
    conf->identity[conf->identity_len] = '\0';

    ret = eap_sm_add_domain_name(conf->identity, &(conf->identity_len));
    if (0 != ret) {
        wpa_printf(MSG_WARNING, "Failed to add domain name in imsi");
        return -1;
    }
...
struct wpabuf * eap_sm_buildIdentity(struct eap_sm *sm, int id, int encrypted)
{
	...
	if (config == NULL) {
 	...
 	//w00176398 add for re-auth & anonymous auth   
       u8 tempId[256] = {'\0'};
       
       struct eap_method_type *m = config->eap_methods;
	int i;
       int aka = 0;
       
       for (i = 0; m && (m[i].vendor != EAP_VENDOR_IETF ||
			  m[i].method != EAP_TYPE_NONE); i++) {
	       wpa_printf(MSG_DEBUG, "m[%d].vendor:%d  m[%d].method:%d",i, m[i].vendor,i,m[i].method);
		if (m[i].vendor == EAP_VENDOR_IETF &&
		    m[i].method == EAP_TYPE_AKA) {
			aka = 1;
			break;
    ...
    {
        eap_deinit_prev_method(sm, "INITIALIZE");
        wpa_printf(MSG_DEBUG, "EAP: wps already build identity");
    }
    else
    {
        if (config->pcsc) {
            ret = eap_sm_get_scard_identity(sm, config);
            if (0 > ret) {
                wpa_printf(MSG_WARNING, "EAP: eap_sm_imsi_identity get null");
            }
        }
    }
    /* END   PN: 2112006806, Modified by l00182072, 2012/11/26*/
	...
	if (sm->m && sm->m->get_identity &&
...
#ifdef  SUPP_EAP_SIM_TEST        
		wpa_printf(MSG_INFO, "EAP: using method re-auth "
				  "identity:%s len:%d", identity, identity_len);
#endif
		...
		wpa_hexdump_ascii(MSG_DEBUG, "EAP: using anonymous identity",
				  identity, identity_len);
#ifdef  SUPP_EAP_SIM_TEST              
		wpa_printf(MSG_DEBUG, "EAP: using anonymous identity:%s len:%d",
				  identity, identity_len);
#endif
	} else {
		...
		wpa_hexdump_ascii(MSG_DEBUG, "EAP: using real identity",
				  identity, identity_len);
#ifdef  SUPP_EAP_SIM_TEST        
		wpa_printf(MSG_INFO, "EAP: using real identity:%s len:%d",
				  identity, identity_len);
#endif
	}
