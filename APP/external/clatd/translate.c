 ...
 *
 *  日期                 单号               修改人           修改原因       
 * 2013-11-14    3110710770     n00202065      CLAT功能下，下挂PC发送的IPv4报文经单板翻译后，TTL减少2
...
/* function: rfc6052_from_ipv6_to_ipv4
 * Translate IPv6 address to IPv4 address according to RFC6052
 * added by m00172998 20130725
 */
int rfc6052_from_ipv6_to_ipv4(struct in6_addr *ipv6, struct ipv6_prefix *prefix, struct in_addr *ipv4)
{
	union ipv4_address dst_aux;

	switch (prefix->len) {
	case 32:
		dst_aux.as32 = ipv6->s6_addr32[1];
		break;
	case 40:
		dst_aux.as8[0] = ipv6->s6_addr[5];
		dst_aux.as8[1] = ipv6->s6_addr[6];
		dst_aux.as8[2] = ipv6->s6_addr[7];
		dst_aux.as8[3] = ipv6->s6_addr[9];
		break;
	case 48:
		dst_aux.as8[0] = ipv6->s6_addr[6];
		dst_aux.as8[1] = ipv6->s6_addr[7];
		dst_aux.as8[2] = ipv6->s6_addr[9];
		dst_aux.as8[3] = ipv6->s6_addr[10];
		break;
	case 56:
		dst_aux.as8[0] = ipv6->s6_addr[7];
		dst_aux.as8[1] = ipv6->s6_addr[9];
		dst_aux.as8[2] = ipv6->s6_addr[10];
		dst_aux.as8[3] = ipv6->s6_addr[11];
		break;
	case 64:
		dst_aux.as8[0] = ipv6->s6_addr[9];
		dst_aux.as8[1] = ipv6->s6_addr[10];
		dst_aux.as8[2] = ipv6->s6_addr[11];
		dst_aux.as8[3] = ipv6->s6_addr[12];
		break;
	case 96:
		dst_aux.as32 = ipv6->s6_addr32[3];
		break;
	default:
        logmsg(ANDROID_LOG_FATAL,"Prefix has an invalid length: %d.", prefix->len);
		//log_err("Prefix has an invalid length: %d.", prefix->len);
		return -1;
	}

	ipv4->s_addr = dst_aux.as32;
	return 0;
}

/* function: rfc6052_from_ipv4_to_ipv6
 * Translate IPv4 address to IPv6 address according to RFC6052
 * added by m00172998 20130725
 */
int rfc6052_from_ipv4_to_ipv6(struct in_addr *ipv4, struct ipv6_prefix *prefix, struct in6_addr *ipv6)
{
	union ipv4_address src_aux;

	src_aux.as32 = ipv4->s_addr;
	memset(ipv6, 0, sizeof(*ipv6));

	switch (prefix->len) {
	case 32:
		ipv6->s6_addr32[0] = prefix->address.s6_addr32[0];
		ipv6->s6_addr32[1] = src_aux.as32;
		break;
	case 40:
		ipv6->s6_addr32[0] = prefix->address.s6_addr32[0];
		ipv6->s6_addr[4] = prefix->address.s6_addr[4];
		ipv6->s6_addr[5] = src_aux.as8[0];
		ipv6->s6_addr[6] = src_aux.as8[1];
		ipv6->s6_addr[7] = src_aux.as8[2];
		ipv6->s6_addr[9] = src_aux.as8[3];
		break;
	case 48:
		ipv6->s6_addr32[0] = prefix->address.s6_addr32[0];
		ipv6->s6_addr[4] = prefix->address.s6_addr[4];
		ipv6->s6_addr[5] = prefix->address.s6_addr[5];
		ipv6->s6_addr[6] = src_aux.as8[0];
		ipv6->s6_addr[7] = src_aux.as8[1];
		ipv6->s6_addr[9] = src_aux.as8[2];
		ipv6->s6_addr[10] = src_aux.as8[3];
		break;
	case 56:
		ipv6->s6_addr32[0] = prefix->address.s6_addr32[0];
		ipv6->s6_addr[4] = prefix->address.s6_addr[4];
		ipv6->s6_addr[5] = prefix->address.s6_addr[5];
		ipv6->s6_addr[6] = prefix->address.s6_addr[6];
		ipv6->s6_addr[7] = src_aux.as8[0];
		ipv6->s6_addr[9] = src_aux.as8[1];
		ipv6->s6_addr[10] = src_aux.as8[2];
		ipv6->s6_addr[11] = src_aux.as8[3];
		break;
	case 64:
		ipv6->s6_addr32[0] = prefix->address.s6_addr32[0];
		ipv6->s6_addr32[1] = prefix->address.s6_addr32[1];
		ipv6->s6_addr[9] = src_aux.as8[0];
		ipv6->s6_addr[10] = src_aux.as8[1];
		ipv6->s6_addr[11] = src_aux.as8[2];
		ipv6->s6_addr[12] = src_aux.as8[3];
		break;
	case 96:
		ipv6->s6_addr32[0] = prefix->address.s6_addr32[0];
		ipv6->s6_addr32[1] = prefix->address.s6_addr32[1];
		ipv6->s6_addr32[2] = prefix->address.s6_addr32[2];
		ipv6->s6_addr32[3] = src_aux.as32;
		break;
	default:
        logmsg(ANDROID_LOG_FATAL,"Prefix has an invalid length: %d.", prefix->len);
		//log_err("Prefix has an invalid length: %d.", prefix->len);
...
/* function: ipv6_dst_to_ipv4_dst
 * return the corresponding ipv4 address for the given ipv6 address
 * destination - ipv4 destination address (network byte order)
 * added by m00172998 20130725
 */
uint32_t ipv6_dst_to_ipv4_dst(const struct in6_addr *destinationaddr, struct ipv6_prefix *prefix) {
  struct in_addr v4_destination;  

  rfc6052_from_ipv6_to_ipv4(destinationaddr, prefix, &v4_destination);  

  return v4_destination.s_addr;
}

/* function: ipv4_src_to_ipv6_src
 * return the corresponding ipv6 address for the given ipv4 address
 * destination - ipv4 source address (network byte order)
 * added by m00172998 20130725
 */
struct in6_addr ipv4_src_to_ipv6_src(uint32_t source, struct ipv6_prefix *prefix) {
  struct in6_addr v6_source;
  struct in_addr v4_source;  

  v4_source.s_addr = source;
  rfc6052_from_ipv4_to_ipv6(&v4_source, prefix, &v6_source);  
                    ...
                    const struct ip6_hdr *old_header) 
{
  ...
  ip->ttl = old_header->ip6_hlim + 1;
  ip->protocol = protocol;
  ip->check = 0;
  
  if ( IPPROTO_FRAGMENT == protocol )
  {
    ip->tot_len = htons(sizeof(struct iphdr) + payload_len - sizeof(struct ip6_frag));
    ip6_frag = (struct ip6_frag *)(old_header + 1);
    ip->id = htons((u_int16_t)(ntohl(ip6_frag->ip6f_ident)));
    if ( IPPROTO_ICMPV6 == ip6_frag->ip6f_nxt)
    {
      ip->protocol = IPPROTO_ICMP;
    }
    else
    {
      ip->protocol = ip6_frag->ip6f_nxt;
    }
    ip->frag_off = htons(ntohs(ip6_frag->ip6f_offlg) >> IPV4_SEGMENT_OFFSET);//todo
    if ( ip6_frag->ip6f_offlg & htons(IP6F_MORE_FRAG) )
    {
...
#ifdef FEATURE_CLAT_USE_RFC6052
  ip->daddr = ipv6_dst_to_ipv4_dst(&old_header->ip6_dst, &Global_Clatd_Config.ipv4_converted_address);    
#else
                     ...
                     const struct iphdr *old_header)
{
  ...
  ip6->ip6_hlim = old_header->ttl + 1;

  frag_flags = ntohs(old_header->frag_off);
  if ( (frag_flags & IP_MF) || (frag_flags & IP_OFFMASK) )
  {
    ip6->ip6_nxt = IPPROTO_FRAGMENT;
    ip6->ip6_plen = htons( payload_len + sizeof(struct ip6_frag));
    ip6_frag = (struct ip6_frag *)(ip6 + 1);
    memset(ip6_frag, 0, sizeof(struct ip6_frag));
    ip6_frag->ip6f_nxt = protocol;
    ip6_frag->ip6f_offlg = htons(ntohs(old_header->frag_off) << IPV4_SEGMENT_OFFSET);
    if ( old_header->frag_off & htons(IP_MF))
    {
      ip6_frag->ip6f_offlg |= htons(IP6F_MORE_FRAG);
    }
    ip6_frag->ip6f_ident = htonl((u_int32_t)ntohs(old_header->id));
  }

#ifdef FEATURE_CLAT_USE_RFC6052
  ip6->ip6_src = ipv4_src_to_ipv6_src(old_header->saddr, &Global_Clatd_Config.ipv4_converted_address);       
#else
