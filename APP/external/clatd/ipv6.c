...
int ipv6_packet(clat_packet out, int pos, const char *packet, size_t len) {
  ...
  if ( IPPROTO_FRAGMENT == protocol )
  {
    ...
    if ( IPPROTO_UDP != ip6_frag->ip6f_nxt )
    {
      logmsg_dbg(ANDROID_LOG_ERROR, "do not support fragment except udp\n");
      return 0;
    }
  }
  ...
  if ( IPPROTO_FRAGMENT == protocol )
  {
    next_header += sizeof(struct ip6_frag);
    protocol =  ip_targ->protocol;
    if ( IPPROTO_UDP != protocol )// only process udp fragment
    {
      return 0;
    }
  }
