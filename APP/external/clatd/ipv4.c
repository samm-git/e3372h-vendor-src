...
int ipv4_packet(clat_packet out, int pos, const char *packet, size_t len) {
  ...
  if (((frag_flags & IP_MF) || (frag_flags & IP_OFFMASK))  
    &&  ( IPPROTO_UDP != header->protocol) ){ // according RFC 6145 only udp fragement supported
  ...
  if ( IPPROTO_FRAGMENT == ip6_targ->ip6_nxt )
  {
    out[pos].iov_len = sizeof(struct ip6_hdr) + sizeof(struct ip6_frag);
  }
  else
  {
  ...
  // Set the length.
  if ((frag_flags & IP_MF) || (frag_flags & IP_OFFMASK)) //add fragment header length if frag
  {
    ip6_targ->ip6_plen = htons(packet_length(out, pos) +  sizeof(struct ip6_frag));
  }
  else
  {
