...
void dns64_detection() {
  ...
  while(1) {
    ...
    if(status > 0) {
      ...
      logmsg(ANDROID_LOG_FATAL, "dns64_detections succeeded\n");  
      return;
    }
    if(status < 0) {
...
int subnet_from_interface(cnode *root, const char *interface) {
  ...
  if(!config_item_ip6(root, "ipv6_host_id", DEFAULT_IPV6_HOST_ID, &Global_Clatd_Config.ipv6_host_id))
  {
    logmsg(ANDROID_LOG_FATAL,"unable to set ipv6_host_id");
    return 0;  
  }    
...
int read_config(const char *file, const char *uplink_interface, const char *plat_prefix) {
  ...
  if(!config_item_ip6(root, "clat_ipv6_address", DEFAULT_CLAT_IPV6_ADDRESS, &Global_Clatd_Config.ipv4_converted_address.address))  
    goto failed;
  
  if(!config_item_int16_t(root, "clat_ipv6_prefixlen", DEFAULT_CLAT_IPV6_PREFIXLEN, &Global_Clatd_Config.ipv4_converted_address.len))  
    goto failed;  
#else
  ...
  if(!config_item_int16_t(root, "ipv6mtu", DEFAULT_IPV6_MTU, &Global_Clatd_Config.ipv6mtu))
    goto failed;

  if(!config_item_int16_t(root, "ipv4mtu", DEFAULT_IPV4_MTU, &Global_Clatd_Config.ipv4mtu))
    goto failed;

  if(!config_item_ip(root, "ipv4_virtual_address", DEFAULT_IPV4_VIRTUAL_ADDRESS, &Global_Clatd_Config.ipv4_local_subnet))
    goto failed;

  if(!config_item_ip(root, "ipv4_vitual_gatway", DEFAULT_IPV4_VIRTUAL_GATEWAY, &Global_Clatd_Config.ipv4_virtual_gatway))
    goto failed;  
  ...
  if(plat_prefix) { // plat subnet is coming from the command line
    ...
    if(!tmp_ptr || strcmp(tmp_ptr, "no") == 0) {
      ...
      if(!config_item_ip6(root, "plat_ipv6_address", DEFAULT_PLAT_IPV6_ADDRESS, &Global_Clatd_Config.plat_subnet)) {
...
void dump_config() {
  ...
  logmsg(ANDROID_LOG_DEBUG,"ipv6mtu = %d",Global_Clatd_Config.ipv6mtu);
