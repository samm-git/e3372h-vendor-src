...
/**********************问题单修改记录******************************************
 ...
 2012.9.18        y00186923     2121101411 fortify告警
******************************************************************************/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
/*****************************************************************************
 函数名称  : br_cmd_setisolate
 功能描述  : 设置两个端口之间隔离的命令行解释函数
 输入参数  : argc 参数个数
             argv 参数内容
 输出参数  : 0执行成功
             1执行失败
...
static int br_cmd_setisolate(int argc, char *const* argv)
{
    const char *brname;
    const char *optype;
    int err;
    ...
    if ((NULL == brname) || (NULL == optype))
    {
        fprintf(stderr, "err paremeter!\n");
        return 1;
    }

    while (argc-- > 0) {
        const char *ifname1 = *++argv;
        const char *ifname2 = *++argv;
        if (!strcmp(optype,"on"))
        {
            err = br_isolate_add_interface(brname, ifname1, ifname2);
        }
        else
        {
            err = br_isolate_del_interface(brname, ifname1, ifname2);
        }
        ...
        default:
            fprintf(stderr, "can't add isolate %s %s to bridge %s: %s\n",
                ifname1,ifname2, brname, strerror(err));
        }
        return 1;
    }
    return 0;
...
static const struct command commands[] = {
    ...
    /* BEGIN PN: 2071808515,Added by l00206601, 2012/9/18*/
    { 4, "setisolate", br_cmd_setisolate,
      "<bridge> {on|off} <device1> <device2>\tturn setisolate on/off between devices" },
    /* END   PN: 2071808515,Added by l00206601, 2012/9/18*/
