...
writepidfile(const char * fname, int pid)
{
	...
	{
		NP_UPNP_ERROR("Unable to open pidfile for writing %s: %s\n", fname, strerror(errno));
		return -1;
	}
	...
	if(pidstringlen <= 0)
	{
        NP_UPNP_ERROR("Unable to write to pidfile %s: snprintf(): FAILED\n", fname);
		close(pidfile);
		return -1;
	}
	else
	{
        ...
        {
            NP_UPNP_ERROR("Unable to write to pidfile %s: %s\n", fname, strerror(errno));
        }
	}
