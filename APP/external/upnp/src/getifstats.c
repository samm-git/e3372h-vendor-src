...
getifstats(const char * ifname, struct ifdata * data)
{
    ...
    {
		NP_UPNP_ERROR("getifstats() : time() error : %s\n", strerror(errno));
	} else {
	...
	if(!f) {
		NP_UPNP_ERROR("getifstats() : cannot open /proc/net/dev : %s\n", strerror(errno));
		return -1;
	}
	/* discard the two header lines */
	if(!fgets(line, sizeof(line), f) || !fgets(line, sizeof(line), f))
    {
		NP_UPNP_ERROR("getifstats() : error reading /proc/net/dev : %s\n", strerror(errno));
	}
	...
	if(f) {
		...
		NP_UPNP_WARN("cannot read %s file : %s\n", fname, strerror(errno));
	}
#ifdef ENABLE_GETIFSTATS_CACHING
