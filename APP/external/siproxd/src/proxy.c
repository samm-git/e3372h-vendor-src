...
static char const ident[]="$Id: proxy.c,v 1.2 2009/02/27 02:34:20 l41296 Exp $";
...
//         return STS_FAILURE;
      ...
      if (i >= URLMAP_SIZE)
      {
         ...
         if (type == REQTYP_OUTGOING)
         {
             ...
             strcpy(urlmap[i].true_url->host, inet_ntoa(from->sin_addr));
#endif            
             
             /* To: field */
             osip_uri_clone( ticket->sipmsg->to->url, 
                    &urlmap[i].reg_url);
             urlmap[i].to_ip = (char*)malloc(32);
 ...
 */
   ...
   else
   {
      ...
      printsip("Sip: Func:proxy_request(), pkt:dst url->host:[%s] \r\n", 
          url->host);
      if ((Hg522 != -1) && (NULL != urlmap[Hg522].to_ip) && (type == REQTYP_OUTGOING))
      {
   ...
   for (media_stream_no=0;;media_stream_no++)
   {
      ...
      if (sdp_message_m_port_get(sdp, media_stream_no))
      {
         ...
         if (msg_port > 0)
         {
            ...
            /*DEBUGC(DBCLASS_PROXY, "client_id: %s; username:%s; host:%s; ip:%s", client_id,
                cont_url->username, cont_url->host, acSdpCaddr);*/
            /* end of AU4D00864 by d00107688 2008-10-17 解决HG323和SIP电话采用非快启无法通话问题 */
