...
static char const ident[]="$Id: register.c,v 1.2 2009/02/27 02:34:20 l41296 Exp $";
...
*/
   ...
   */
   ...
   printsip("Sip: The sipmsg to->username:[%s], to->host[%s] \r\n",
                   (url1_to->username) ? url1_to->username : "*NULL*",
   ...
   if (expires > 0) {
      ...
      for (i=0; i<URLMAP_SIZE; i++) {
	    ...
	    if (j < 0) j=i; /* remember first hole */
         ...
         if (compare_url(url1_to, url2_to)==STS_SUCCESS) {
            printsip("found entry for %s@%s <-> %s@%s at "
                   "slot=%i, exp=%li \r\n",
	           (url1_contact->username) ? url1_contact->username : "*NULL*",
                   (url1_contact->host) ? url1_contact->host : "*NULL*",
      ...
      if (i >= URLMAP_SIZE) {
         ...
         printsip("create new entry for %s@%s <-> %s@%s at slot=%i \r\n",
                (url1_contact->username) ? url1_contact->username : "*NULL*",
                (url1_contact->host) ? url1_contact->host : "*NULL*",
      ...
      } else { /* if new entry,else we have a record to recove it */
      /*
    ...
    * un-REGISTER
    */
   } else { /* expires > 0 , else expires <= 0 */
      /*
       * Remove registration
       * Siproxd will ALWAYS remove ALL bindings for a given
       * address-of-record
       */
      printsip("Sip:Sip:register_client() expires <= 0 \r\n");
      for (i=0; i<URLMAP_SIZE; i++) {
         ...
         if (compare_url(url1_to, url2_to)==STS_SUCCESS) {
            printsip("removing registration for %s@%s at slot=%i",
	           (url2_to->username) ? url2_to->username : "*NULL*",
