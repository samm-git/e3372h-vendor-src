...
static char const ident[]="$Id: rtpproxy_relay.c,v 1.1 2008/02/19 07:51:58 y42304 Exp $";
      ...
      if(g_pthread_out)
     {
      ...
      for (i=0;(i<RTPPROXY_SIZE) && (num_fd>0);i++) 
      {
        ...
        /* read from sock rtp_proxytable[i].sock*/
            ...
            struct sockaddr_in *pfrom = NULL;
            socklen_t fromlen = sizeof(from);
            count = recvfrom(rtp_proxytable[i].rtp_rx_sock, rtp_buff, RTP_BUFFER_SIZE, 0,
                    &from, &fromlen);

            pfrom = (struct sockaddr_in *)&from;
            printf("#xc#recv from: %s port %d len: %d\n",
                utils_inet_ntoa(pfrom->sin_addr),
...
#else				  
				  printf("#xc#sendto: %s port %d len: %d\n",
                         ...
                         int remote_port)          //  m= 选项中的port值
{
   ...
   /*BEGIN 3082101267 00201037 20130823 modified*/
   osip_time(&rtp_proxytable[freeidx].timestamp);
   /*END 3082101267 00201037 20130823 modified*/
