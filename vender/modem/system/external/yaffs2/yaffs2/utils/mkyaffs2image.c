...
static int write_chunk(__u8 *data, __u32 objId, __u32 chunkId, __u32 nBytes)
{
	...
	//add chenpeng********
	struct mtd_oob_ops1 ops ;
	char spareok[spareSize];

	uint8_t *oob = ops.oobbuf = (uint8_t*)spare;
	uint8_t *ok = (uint8_t*)spareok;
	ops.ooboffs = 0;
	int len = (sizeof(yaffs_PackedTags2));   //28???;
	memset(spareok, 0xff, spareSize);
...
//add chenpeng***********
	...
	struct nand_oobfree *free =nand_oob_64.oobfree;
	...
	for(; free->length && len; free++, len -= bytes) {
		...
		memcpy(ok + boffs, oob, bytes);
		oob += bytes;
	}
	return write(outFile,spareok, spareSize);
