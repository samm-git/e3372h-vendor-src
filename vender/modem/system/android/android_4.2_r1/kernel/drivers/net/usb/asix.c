 ...
 */
/******************************************************************************************************
DTS2014021206815    2014-02-20    hujuanli 131015    AF18代码二进制归一
DTS2014051303053    2014-05-19    yuhao 84013175     单板插入AF18自动模式或只usb方式连接pc，同时wifi接入单板，wif
ioffload接入同网关地址的AP，ip地址不更新 
DTS2014070903692    2014-07-23    hujuanli 131015   网口上报事件代码调整
...
#define DRV_VERSION    "4.11.0"

struct usbnet *g_usbnet_dev = NULL;
...
struct AX88772B_BULKIN_SIZE_STRU AX88772B_BULKIN_SIZE[] = {
    /* 2k */
    {2048, 0x8000, 0x8001},
    /* 4k */
    {4096, 0x8100, 0x8147},
...
struct CICADA_FAMILY_HWINIT_STRU CICADA_FAMILY_HWINIT[] = {
...
struct AGERE_FAMILY_HWINIT_STRU AGERE_FAMILY_HWINIT[] = {
...
int usb_set_mac_addr(struct net_device* net)
{
    ...
    {
        deverr(dev, "usb set mac adrr error  net=%d\n", net);
        return -1;
    }
    if (NULL != dev )
    {
    ...
    {
        deverr(dev, "usb set mac adrr error  dev=%d\n", dev);
        return -1;
    }
}
...
static void ax88772_status(struct usbnet *dev, struct urb *urb)
{
    ...
    printk(KERN_ERR"ax88772_status is %6s \r\n",link?"insert":"remove");
...
/*BEGIN: h00131015 2014-04-24  modifify for DTS2014041805609 */
...
/*END: h00131015 2014-04-24  modifify for DTS2014041805609 */
static void ax88772b_status(struct usbnet *dev, struct urb *urb)
{
    ...
    if (netif_carrier_ok(dev->net) != link) {
        if (link) {
            ...
            printk(KERN_ERR"CRADLE_INSERT\n");
            mbb_usbnet_net_state_notify(CRADLE_INSERT);
        } else {
            printk("KERN_ERRCRADLE_REMOVE\n");
...
void mbb_cradle_power_reset(void)
{
    if(g_usbnet_dev)
        {
            ax88772a_phy_powerup(g_usbnet_dev);
        }
}
...
static const struct driver_info ax88772b_info = {
    ...
    .unbind = ax88772b_unbind,
    .status = ax88772b_status,
    .flags = FLAG_ETHER | FLAG_FRAMING_AX | FLAG_HW_IP_ALIGNMENT|IFF_PROMISC,
    .rx_fixup = ax88772b_rx_fixup,
    .tx_fixup = ax88772b_tx_fixup,
    .stop = ax88772b_stop,
    .reset =  ax88772b_reset,
...
{
    printk(KERN_INFO "asix_init Enter\r\n");
