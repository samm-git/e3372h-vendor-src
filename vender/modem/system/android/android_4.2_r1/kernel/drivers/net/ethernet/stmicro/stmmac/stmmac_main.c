/*******************************************************************************
  ...
  2014-09-17 - adapted for Hisi_Balong in stmmac_adjust_link stmmac_init_phy
  display_ring init_dma_desc_rings stmmac_tx stmmac_dma_interrupt stmmac_mmc_setup
  stmmac_get_synopsys_id stmmac_selec_desc_mode stmmac_check_ether_addr stmmac_open
  stmmac_release stmmac_xmit stmmac_rx_refill stmmac_rx stmmac_poll stmmac_tx_timeout
  stmmac_config stmmac_change_mtu stmmac_interrupt stmmac_ioctl stmmac_init_fs
  stmmac_exit_fs stmmac_hw_init stmmac_dvr_probe stmmac_dvr_remove stmmac_suspend
  stmmac_resume stmmac_cmdline_opt Zhongshun Wang <foss@huawei.com>.
...
int stmmac_suspend(struct net_device *ndev)
{
    ...
    if ( eth_power_down() )
    {
        //使用freeze接口替换，解决gmac深睡后中断持续触发问题
        return stmmac_freeze(ndev);
    }
    else
    {
        //如果返回0，说明phy此前已下电，则stmmac无须再执行suspend语句
        return 0;
    }
    #endif
