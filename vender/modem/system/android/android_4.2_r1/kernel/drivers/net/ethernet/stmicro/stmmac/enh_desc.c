/*******************************************************************************
  ...
  2014-06-01 - adapted for Hisi_Balong in enh_desc_release_tx_desc enh_desc_close_tx_desc
  enh_desc_dis_rx_ic enh_desc_ops Fangyuan Liu <foss@huawei.com>.
...
static void enh_desc_close_tx_desc(struct dma_desc *p, int irq_enable)
{
	...
	else if(irq_enable == 1)
	{
        p->des01.etx.interrupt = 1;
	}
	else
	{
        printk("wrong int irq_enable : %d\n", irq_enable);
	}
}
...
static void enh_desc_dis_rx_ic(struct dma_desc *p, int disable_rx_ic)
{
    if(disable_rx_ic)
        p->des01.erx.disable_ic = 1;
}
...
const struct stmmac_desc_ops enh_desc_ops = {
	...
	.get_rx_frame_len = enh_desc_get_rx_frame_len,
 	.dis_rx_intr_comp = enh_desc_dis_rx_ic,
};
