 ...
 * 2013-04-15 - USB3.0 OTG Implementation for hisi balong platform
 * Zhongshun Wang, Xinli Wang<foss@huawei.com>
 *
...
static void usb_gadget_remove_driver(struct usb_udc *udc)
{
	...
	if (udc_is_newstyle(udc)) {
		...
		/* bugfix: cdev is freed in unbind
		 *  when ep0 req in queue and then remove the usb,
		 *  kernel will crash in composite_setup_complete
		 */
...
struct usb_gadget *usb_udc_get_gadget(struct usb_gadget_driver *driver)
{
	struct usb_udc *udc = NULL;
...
EXPORT_SYMBOL_GPL(usb_udc_get_gadget);
...
found:
	USB_DBG_GADGET(&udc->dev, "registering UDC driver [%s]\n",
			driver->function);
