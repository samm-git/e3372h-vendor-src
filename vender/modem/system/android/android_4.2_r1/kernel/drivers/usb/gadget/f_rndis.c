/*lint -save -e34 -e63 -e578 -e573 -e52 -e831 -e413 -e132 -e101 -e2 -e565 -e84 -e516 
-e43 -e550 -e62 -e64 -e648 -e616 -e30 -e529 -e665 -e123 -e958 -e651 -e124 -e18 
-e650 -e539 -e142 -e527 -e529 -e533 -e537 -e713 -e718 -e730 -e732 -e734 -e740 -e746*/
/*
 ...
 * 2013-08-17 - Adopt&bugfix for hisi balong platform 
 *		Add&Change function:
 *			bitrate
 *			rndis_wrap_single
 *			rndis_unwrap_array
 ...
 * Zhongshun Wang, Yong Jing, Xinli Wang, Qiang Yang, Changdong Fan<foss@huawei.com>
 *
					...
					struct sk_buff *skb)
{
	...
	if (skb)
	{
		...
		pad_len = ALIGN(skb->len + RNDIS_PACKET_HEADER_LEN, GNET_ALIGN_LEN) - skb->len - RNDIS_PACKET_HEADER_LEN;
...
static int rndis_unwrap_array(struct gether *port,
			  struct sk_buff *skb,
			  struct sk_buff_head *list)
{
	...
	GNET_TRACE(("%s: 100: enter,total_len=%d\n", __FUNCTION__,total_len));
...
static void rndis_suspend(struct usb_function *f)
{
    ...
    if ((!rndis->curr_alt) && (!dev)) {
        GNET_INFO(("rndis function disabled, skip the rndis adapter suspend process\n"));
        return;
    }
...
static void rndis_resume(struct usb_function *f)
{
    ...
    if ((!rndis->curr_alt) && (!dev)) {
        GNET_INFO(("rndis function disabled, skip the rndis adapter resume process\n"));
        return;
    }
...
rndis_bind(struct usb_configuration *c, struct usb_function *f)
{
    ...
    bsp_usb_add_setup_dev((unsigned)status);
	...
	USB_DBG_GADGET(&cdev->gadget->dev, "RNDIS: %s speed IN/%s OUT/%s NOTIFY/%s\n",
...
static inline void rndis_reset_values(struct f_rndis *gadget_rndis)
{
	gadget_rndis->port.fixed_out_len = GNET_OUT_MAXSIZE_PER_TRANSFER;
	gadget_rndis->port.fixed_in_len = GNET_IN_MAXSIZE_PER_TRANSFER;
				...
				u32 vendorID, const char *manufacturer)
{
	...
	rndis->port.unwrap = rndis_rm_hdr;
#endif	/* end of CONFIG_BALONG_RNDIS */
