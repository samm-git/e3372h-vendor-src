 ...
 * 2014-2-12 - clear coverity warning jingyong <foss@huawei.com>
 *
		...
		u16 wIndex, char *buf, u16 wLength)
{
	...
	case SetPortFeature:
		...
		case USB_PORT_FEAT_SUSPEND:
			...
			temp = xhci_readl(xhci, port_array[wIndex]);/* [false alarm]:fortify disable */
			bus_state->suspended_ports |= 1 << wIndex;
			break;
...
int xhci_bus_resume(struct usb_hcd *hcd)
{
	...
	temp |= CMD_EIE;
	xhci_writel(xhci, temp, &xhci->op_regs->command);
	temp = xhci_readl(xhci, &xhci->op_regs->command);/* [false alarm]:fortify disable */
