...
{
#ifdef USB_SECURITY
    if (SHELL_OPEN_FLAG ==  get_shell_lock())
    {
        DBG_I(MBB_ACM,"gs_acm_is_console_enable return 1\n");
        return 1;
    }
    else
    {
        DBG_I(MBB_ACM,"gs_acm_is_console_enable return 0\n");
        return 0;
    }
#else
    /*默认USB shell 功能不可用*/
    DBG_I(MBB_ACM,"gs_acm_is_console_enable return 00\n")
    return 0;
#endif
}
...
USB_INT bsp_usb_console_init(USB_VOID)
{
    ...
    /*w84014342*/
    gs_console_tty_driver->name = acm_ctx->tty_name_type_var[ACM_CONSOLE_IDX].name;
