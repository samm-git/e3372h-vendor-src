...
#include "drv_comm.h"
#include <drv_nv_id.h>
#include "coul_firmware.h"

#define BATTERY_SN (32)
#define BATTERY_TYPE_LEN_MIN (2)
#define BATTERY_TYPE_LEN_MAX (3)
#define BATTERY_TYPE_START_LEN (4)
#define COUL_FILE_PATH_MAX (50)
#define COUL_THREAD_RET (3)

#define COUL_FIRMWARE_UPDATE_LEN (191278)

#define NV_HUAWEI_BATTERY_INFO_I (50535)
typedef struct
{
...
typedef enum
{
    en_battery_type_none,
    en_battery_type_scud,
    en_battery_type_sunwoda,
    en_battery_type_max
}battery_type_enum;
battery_type_enum  e5770s_battery_type = en_battery_type_none;
extern bool  nv_revert_flag;
bool  coul_update_start_flag  = false;
bool  coul_update_status_flag = false;
#endif
 ...
 */
/*i2c device info*/
static struct i2c_board_info i2c_bq27510_info[] = {
    {
        I2C_BOARD_INFO(BQ27510_NAME, BSP_NORMAL_MODE_I2C_ADDR),
        .irq = BATT_INT_PIN,
    },
};
...
typedef struct 
{
    battery_id_type battery_id;
    const char *name;
}battery_fw;

battery_fw battery_fw_table[]={
{BAT_ID_10K_STATUS,"BQ27510_FMT_1500"},
{BAT_ID_22K_STATUS,"BQ27510_FMT_1900"},
{BAT_ID_39K_STATUS,"BQ27510_FMT_2400"},
{BAT_ID_68K_STATUS,"BQ27510_FMT_3000"},
{BAT_ID_110K_STATUS,"BQ27510_XWD_1500"},
{BAT_ID_200K_STATUS,"BQ27510_XWD_1900"},
{BAT_ID_470K_STATUS,"BQ27510_XWD_2400"},
{BAT_ID_2M_STATUS,"BQ27510_XWD_3000"},
};
/**************************************************/
...
//stub
bool get_hw_config_int(unsigned char* name,unsigned int* value,unsigned int* option)
{
...
int bq27510_get_gpadc_conversion(int channel_no)
{
    ...
    ret = bsp_hkadc_convert(channel_no,&value);
    if(ret)
    {
        printk("get batt id volt error\n");
        return -1;
    }
...
int get_battery_id(void)
{
    ...
    data = bq27510_get_gpadc_conversion(BATT_ID_HKADC_ID);
...
static bool bq27510_get_firmware_name(char *config_name)
{
    int index = 0;

    for (index = 0; index < sizeof(battery_fw_table); index++)
    {
        if(battery_id_status == battery_fw_table[index].battery_id)
        {
            break;
        }
    }
    strncpy(config_name,battery_fw_table[index].name,sizeof(battery_fw_table[index].name));
...
#if(FEATURE_ON == MBB_CHG_BQ27510)
    unsigned short ret_coul = NV_ERROR;
...
#if(FEATURE_ON == MBB_CHG_BQ27510)
    printk("[%s] LINE [%d]  iRet =%d \r\n",__FUNCTION__,__LINE__,iRet);
    ret_coul = bsp_nvm_read(NV_HUAWEI_BATTERY_INFO_I,\
                           &coul_firmware_update_info,\
                           sizeof(nv_huawei_coul_firmware_update_info));
    if((iRet == NV_OK) && (NV_OK == ret_coul))
    {
        coul_firmware_update_info.coul_firmware_update_status = 1;
    }
    ret_coul = bsp_nvm_write(NV_HUAWEI_BATTERY_INFO_I,\
                                         &coul_firmware_update_info,\
                                         sizeof(nv_huawei_coul_firmware_update_info));
    if(NV_OK != ret_coul)
    {
         printk("[%s] : LINE[%d]  write nv ret_coul =%d\r\n",__FUNCTION__,__LINE__,\
...
#if(FEATURE_OFF == MBB_CHG_BQ27510)
    oldfs = get_fs();
    set_fs(KERNEL_DS);
...
#endif
#if(FEATURE_ON == MBB_CHG_BQ27510)
    length = COUL_FIRMWARE_UPDATE_LEN;
    printk(KERN_ERR "[%s,%d]  e5770s_battery_type =%d\n",\
                __FUNCTION__,__LINE__,e5770s_battery_type);
    switch(e5770s_battery_type)
    {
        case en_battery_type_scud:
            buf = bq27510_cmd_buffer_lg_ver1_0;
        break;
        case en_battery_type_sunwoda:
            buf = bq27510_cmd_buffer_sanyo_ver1_0;
        break;
        default:
            return  1;
    }
...
static ssize_t bq27510_attr_store(struct device_driver *driver,const char *buf, size_t count)
{
    ...
    nv_huawei_coul_firmware_update_info coul_firmware_update_info = {0x00};
    unsigned short ret_coul = NV_ERROR;
    unsigned char battery_info[BATTERY_TYPE_LEN_MAX] = {0x00};
#endif
    ...
    if((path_image[count-1]) == '\n')
        ...
        path_image[count] = '\0';
#if(FEATURE_ON == MBB_CHG_BQ27510)
    printk("*************[%s] :begin updata**********\n",__FUNCTION__);

    ret_coul = bsp_nvm_read(NV_HUAWEI_BATTERY_INFO_I,\
                           &coul_firmware_update_info,\
                           sizeof(nv_huawei_coul_firmware_update_info));
    if((0 != ret_coul) || (strlen(coul_firmware_update_info.battery_sn) == 0))
    {
        printk("[%s] LINE [%d]read NV error or No battery SN\n",__FUNCTION__,__LINE__);
        return 1;
    }

    if(coul_firmware_update_info.coul_firmware_update_status == 1)
    {
        printk("[%s] LINE [%d] this battery have been update no need update\n",\
                __FUNCTION__,__LINE__);
        coul_update_status_flag = true;
        return 0;
    }
    strncpy(&battery_info,\
            &coul_firmware_update_info.battery_sn[BATTERY_TYPE_START_LEN],\
              BATTERY_TYPE_LEN_MIN);

    printk("[%s] : LINE[%d]  ret_coul =%s\r\n",__FUNCTION__,__LINE__,\
                         coul_firmware_update_info.battery_sn);
    printk("[%s] : LINE[%d]  ret_coul =%d\r\n",__FUNCTION__,__LINE__,\
                         coul_firmware_update_info.coul_firmware_update_status);
    printk("[%s] : LINE[%d]  battery_info =%s\r\n",__FUNCTION__,__LINE__,\
                           battery_info);

    if(strstr(battery_info,"LC") != NULL)
    {
        e5770s_battery_type = en_battery_type_scud;
    }
    else if(strstr(battery_info,"YI") != NULL)
    {
        e5770s_battery_type = en_battery_type_sunwoda;
    }
    else
    {
        printk("[%s] LINE [%d] this battery not valid\n",__FUNCTION__,__LINE__);
        coul_update_status_flag = true;
        e5770s_battery_type = en_battery_type_max;
        return 0;
    }
    
    printk("[%s] LINE [%d] this battery type =%d\n",__FUNCTION__,__LINE__,\
                        e5770s_battery_type);
#endif
...
ssize_t bq27510_update_test(void)
{
    int   length = 0;
    char   path[COUL_FILE_PATH_MAX] = {"/etc/LGC.bqfs"};
    length = strlen(path);
    bq27510_attr_store(&bq27510_battery_driver.driver,path,length);
    return 0;
}
ssize_t bq27510_coul_firmware_update_init(void)
{
    int pid;
    int iRet = COUL_THREAD_RET;
    uint32_t real_factory_mode = 1;

    printk("kernel_thread begin to  iRet =%d ++++++++++++\r\n",iRet);
    if(nv_revert_flag)
    {

        bsp_nvm_read(NV_ID_DRV_VER_FLAG,\
                           &real_factory_mode,\
                           sizeof(uint32_t));
        printk("NV_FACTORY_MODE_I real_factory_mode =%d ++++++++++++\r\n",\
                           real_factory_mode);

        if(real_factory_mode)
        {
           iRet = kernel_thread(bq27510_update_test, NULL, CLONE_FS | CLONE_SIGHAND);
           nv_revert_flag = false;
           coul_update_start_flag = true;
        }
    }
    printk("kernel_thread end to  iRet =%d ++++++++++++\r\n",iRet);
    return iRet;
}
bool bq27510_get_coul_update_start_flag(void)
{
    return coul_update_start_flag;
}
bool bq27510_get_coul_update_status_flag(void)
{
    return coul_update_status_flag;
}
void bq27510_set_coul_update_start_flag(bool state)
{
    coul_update_start_flag = state;
    return;
}
void bq27510_set_coul_update_status_flag(bool state)
{
    coul_update_status_flag = state;
    return;
}
EXPORT_SYMBOL_GPL(bq27510_update_test);
EXPORT_SYMBOL_GPL(bq27510_coul_firmware_update_init);
EXPORT_SYMBOL_GPL(bq27510_get_coul_update_start_flag);
EXPORT_SYMBOL_GPL(bq27510_get_coul_update_status_flag);
EXPORT_SYMBOL_GPL(bq27510_set_coul_update_start_flag);
EXPORT_SYMBOL_GPL(bq27510_set_coul_update_status_flag);
#endif
              ...
              struct device_attribute *attr,char *buf)
{
    ...
    val = bq27510_get_gpadc_conversion(BATT_ID_HKADC_ID);
                                 ...
                                 const struct i2c_device_id *id)
{
    ...
    coul_ops->charger_event_rcv = bq_charger_event_rcv;/*充电事件处理函数*/
...
static int __init bq27510_device_init(void)
{
    int ret = 0;
    //choose the right master,now master0
    ret = i2c_register_board_info(0, i2c_bq27510_info, ARRAY_SIZE(i2c_bq27510_info));
    if(ret)
    {
        printk(KERN_ERR "bq27510_device_init failed!!!\n");   
    }    
    return ret;
}
...
postcore_initcall(bq27510_device_init);
