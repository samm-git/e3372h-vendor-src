/*
 * Copyright 漏 1999-2010 David Woodhouse <dwmw2@infradead.org>
 *
			...
			loff_t *ppos)
{
    ...
    {
        if(FLASH_PAGE_SIZE_4096 == mtd->writesize)
        {
            sparesize = ((mtd->writesize >> FLASH_CODEWORD_SHIFT) 
        ...
        {
            count = mtd->size + blocksparesize - *ppos;
        }
    }
#else
    if (*ppos + count > mtd->size)
	...
	while (count) {
        ...
        default:
        {
            ret = mtdchar_read_dload(mtd, *ppos, kbuf, len, &retlen, &ppos_realy);
        }
#else
		default:
			...
			loff_t *ppos)
{
...
#if(MBB_REB == FEATURE_ON)
    reb_write_partition_beyond(*ppos, &size, mtd->name, mtd->size ); 
    if(!size)
    {
        return 0;
    }
	...
	while (count) {
        ...
        default:
        { 
            ret = mtdchar_write_dload(mtd, *ppos, kbuf, len, &retlen, &ppos_realy);
        }
#else
		default:
...
#if(FEATURE_ON == MBB_ECCSTATUS_OPTIMIZE)
	case MSM_NAND_ECCGETSTATS:
	{
        struct mtd_part *part = PART(mtd);
        if (copy_to_user(argp,&part->master->ecc_stats,
