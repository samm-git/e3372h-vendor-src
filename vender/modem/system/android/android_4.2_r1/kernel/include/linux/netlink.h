/*
 * 2013-05-07 - sync jellybean drv to ci. Copyright (C) Huawei Technologies Co., Ltd. 
...
typedef enum _DEVICE_ID
{
    ...
    DEVICE_ID_TIMER =  TIMER_ID, 
       ...
       DEVICE_ID_TRAFFIC  =   25,   //作为网口流量监控，需要与eUAP的接口保持一致
#endif
...
typedef enum _USB_EVENT
{
    ...
    USB_CRADLE_UP, /* 当在CRADLE上枚举成功时，上报此事件 */
    USB_CRADLE_DOWN, /* 当从CRADLE上拔出USB设备时，上报此事件 */
	USB_SAMBA_PRINT_ATTACH = 11, /* Samba 打印机插入事件 */
	USB_SAMBA_PRINT_DETACH = 12, /* Samba 打印机拔出事件 */	
    USB_CRADLE_ATTACH = 13,
    USB_CRADLE_REMOVE = 14,
    USB_CRADLE_PLUGIN = 15,/*cradle 设备插入，  进host 枚举前上报此事件 */
    USB_CRADLE_UNPLUG = 16,/*cradle 设备拔出*/
#endif
	USB_EVENT_MAX
} USB_EVENT;
typedef enum _CRADLE_SPEED
{
    /*无效值*/
    CRADLE_SPEED_INVAILD = -1,
    CRADLE_SPEED_UNKOWN = 0,
    CRADLE_SPEED_1000M = 1,
    CRADLE_SPEED_100M = 2,
    CRADLE_SPEED_10M = 3,
