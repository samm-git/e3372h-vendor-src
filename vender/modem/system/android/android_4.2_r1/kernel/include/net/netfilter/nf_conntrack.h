...
#define FASTIP_FLAGS_CACHED    (1 << 31)	/* Indicates cached connection */
#define FASTIP_FLAGS_EXCLUDED    (1 << 30)
#define FASTIP_FLAGS_REPLY_CACHED    (1 << 1)
#define FASTIP_FLAGS_ORG_CACHED    (1 << 0)
#endif
...
#if (FEATURE_ON == MBB_FEATURE_FASTIP)
    /* 超时计时 */
    u_int32_t fastip_expire_jiffies;
    /* fastip 标记 */
    u_int32_t fastip_flag;
...
/* added by f00104094, define the func type */
typedef void (*NatEntryUpdate)(struct sk_buff *skb, u_int32_t hooknum,
		struct nf_conn *ct,enum ip_conntrack_info ci,
		struct nf_conntrack_tuple *ctuple);
typedef void (*NatEntryDelete)(struct nf_conn *ct);
#endif
