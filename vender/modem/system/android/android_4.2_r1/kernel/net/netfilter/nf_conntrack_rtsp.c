...
#define DEBUGPC(fmt, args...) printk("nf_conntrack_rtsp.c: %s: %d: " fmt, __func__, __LINE__, ## args)
#else
                                     ...
                                     if (ptran[off] == '-') {  
                                             ...
                                             if ((prtspexp->loport & 0x0001) != 0 ||  
                                                 prtspexp->hiport != prtspexp->loport+1) {  
                                                     DEBUGPC("incorrect range: %hu-%hu, correcting\n",  
...
{  
     ...
     while (dataoff < datalen) {  
             ...
             if (expinfo.loport == 0) {  
                     DEBUGPC("no udp transports found\n");  
                     continue;   /* no udp transports found */  
             }  

             DEBUGPC("udp transport found, ports=(%d,%hu,%hu)\n",  
             ...
             if (expinfo.pbtype == pb_range) {  
                     DEBUGPC("Changing expectation mask to handle multiple ports\n");  
                     //exp->mask.dst.u.udp.port  = 0xfffe;  
             }  

             DEBUGPC("expect_related %pI4:%u-%pI4:%u\n",  
                    &exp->tuple.src.u3.ip,  
