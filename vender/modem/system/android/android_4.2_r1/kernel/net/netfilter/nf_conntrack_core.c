 ...
 * 2013-05-09 - add nat sa.
 * 	1 migrated the nat sa code from v7r1 to v7r2, ftp data tested OK
 *  2 support v7r1 also
 *  3 disabled default, maybe enabled by ecall nat_sa_set_flags
 *  fanchangdong <foss@huawei.com>
 *
...
#if (FEATURE_ON == MBB_CTF_COMMON)
#ifdef CONFIG_IPV6
#define IPVERSION_IS_4(ipver) (4 == (ipver))
#else
...
#if(FEATURE_ON == MBB_FEATURE_FASTIP)
extern void fastip_ip_conn_cachet_add(struct sk_buff *skb, u_int32_t hooknum,
                    struct nf_conn *ct, enum ip_conntrack_info ci,
                    struct nf_conntrack_tuple *manip);
extern int fasip_ip_conn_cachet_delete(struct nf_conn *ct, int ct_timeout);
#endif
...
/* added by fanchangdong */
extern int sa_enable;
NatEntryDelete g_pNatEntryDelete=NULL;
EXPORT_SYMBOL_GPL(g_pNatEntryDelete);
NatEntryUpdate g_pNatEntryUpdate=NULL;
EXPORT_SYMBOL_GPL(g_pNatEntryUpdate);
/* end of added by fanchangdong */
#endif
...
#define HTTP_PORT     20480   /*80端口经过转换成20480*/
#endif

bool ip_conntrack_is_ipc_allowed(struct sk_buff *skb, u_int32_t hooknum)
{
    ...
    if (NF_INET_PRE_ROUTING == hooknum || NF_INET_POST_ROUTING == hooknum) {
        dev = skb->dev;
...
void ip_conntrack_ipct_add(struct sk_buff *skb, u_int32_t hooknum,
    struct nf_conn *ct, enum ip_conntrack_info ci, struct nf_conntrack_tuple *manip)
{
    ...
    {
        printk(KERN_WARNING "IP version err %u\n", ipver);
        return;
    }
    ...
    */
    ...
    {
#ifdef CONFIG_PRSITE     
        /*如果是http协议 需要判断强制门户的helper*/
        if ((HTTP_PORT == tcph->source || HTTP_PORT == tcph->dest) && (!g_stPrsiteUrlInfo.lEnable))
        {
            if ((ct->ctf_flags & CTF_FLAGS_EXCLUDED) 
                || ((hooknum != NF_INET_PRE_ROUTING) && (hooknum != NF_INET_POST_ROUTING)))
            {
                return;
            }
    ...
    if (NULL == skb_dst(skb)) {
        if (IPVERSION_IS_4(ipver)) {
            u_int32_t daddr;
            if((manip != NULL) && (NF_NAT_MANIP_DST == HOOK2MANIP(hooknum)))
            {
    ...
    if ((NULL == rt) || (
            ...
            (NULL == rt->dst._neighbour) ||
            (0 == (rt->dst._neighbour->nud_state &
                (NUD_PERMANENT | NUD_REACHABLE | NUD_STALE | NUD_DELAY | NUD_PROBE))))
    {
        return;
    }
    ...
    if (hh != NULL) {
        ...
        memcpy(ipc_entry.dhost.octet, rt->dst._neighbour->ha, ETH_ALEN);
    ...
    if (manip != NULL) {
        if (NF_NAT_MANIP_SRC == HOOK2MANIP(hooknum)) {
    ...
    if (IPVERSION_IS_4(ipver))
    {
        printk(KERN_INFO "%s: Adding ipc entry for [%d]%u.%u.%u.%u:%u - %u.%u.%u.%u:%u\n", __FUNCTION__,
    ...
    printk(KERN_INFO "[%d] vid: %d action %x\n", hooknum, ipc_entry.vid, ipc_entry.action);
    if (manip != NULL)
    {
        printk(KERN_INFO "manip_ip: %u.%u.%u.%u manip_port %u\n",
...
int ip_conntrack_ipct_delete(struct nf_conn *ct, int ct_timeout)
{
    ctf_ipc_t *ipct;
...
#ifdef DEBUG
    printk(KERN_INFO "%s: Deleting the tuple %x %x %d %d %d\n",
...
#if (FEATURE_ON == MBB_FEATURE_FASTIP)
    fasip_ip_conn_cachet_delete(ct, 0);
#endif

	#if defined(CONFIG_NAT_SA) || defined(CONFIG_NAT_SA_MODULE)
	if(g_pNatEntryDelete)
		g_pNatEntryDelete(ct);
	#endif
...
static void death_by_timeout(unsigned long ul_conntrack)
{
...
#if (FEATURE_ON == MBB_FEATURE_FASTIP)
    if (fasip_ip_conn_cachet_delete(ct, jiffies>=ct->timeout.expires ? 1 : 0) != 0)
    {
        return;
    }
#endif
	...
	#if defined(CONFIG_NAT_SA) || defined(CONFIG_NAT_SA_MODULE)
	if(sa_enable && g_pNatEntryDelete)
		g_pNatEntryDelete(ct);
	#endif
...
#if (FEATURE_ON == MBB_FEATURE_FASTIP)
        ct->fastip_expire_jiffies = extra_jiffies;
#endif
