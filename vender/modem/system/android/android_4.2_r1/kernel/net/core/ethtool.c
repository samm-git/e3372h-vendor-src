 ...
 * 2013-06-05 - optimise the cloned skb process in the network stack.
 * 2013-06-24 - add function to show the qdisc status. fanchangdong <foss@huawei.com>
 *
...
static const char netdev_features_strings[NETDEV_FEATURE_COUNT][ETH_GSTRING_LEN] = {
    ...
    [NETIF_F_NOCLONE_CHECK_BIT] =    "no-clone-check",
};
...
void dump_qdisc_stat()
{
    ...
    struct gnet_stats_queue *qstat;

    printk("  name    qlen backlog   drops requeues overlimits\n");

    for_each_netdev(&init_net,net) {
        qstat = &net->qdisc->qstats;

        printk("%6s %7u %7u %7u %7u %7u\n",
        net->name, qstat->qlen, qstat->backlog, qstat->drops, qstat->requeues, qstat->overlimits);
    }
}
