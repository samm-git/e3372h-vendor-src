...
static inline int should_drop_arp_pakcet(struct sk_buff *skb)
{
    ...
    /* 如果是从wan0接口下来的ARP 包,而且目的IP或是源IP与BR0 IP相同的，则将其丢弃*/
    if (('w' == skb->dev->name[0] && 'a' == skb->dev->name[1] && 'n' == skb->dev->name[2] && '0' == skb->dev->name[3]))
    {
		...
		if (brdev)
		{
		    ...
		    if (indev && (NULL != (ifa = indev->ifa_list)))
		    {
		        ...
		        {
		            if (((ifa->ifa_local & ifa->ifa_mask) == ((*((unsigned int*)arp_ptr)) & ifa->ifa_mask))
		                         || ((ifa->ifa_local & ifa->ifa_mask) == ((*((unsigned int*)arp_ptr2)) & ifa->ifa_mask)))
		            {
		                return true;
		            }
		        }
