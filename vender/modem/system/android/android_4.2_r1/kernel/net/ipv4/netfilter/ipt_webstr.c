                                        ...
                                        unsigned int *numlen)
{
    ...
    *numoff = *numlen = 0;
    ...
    {   /* Short packet: try for partial? */
        if (0 == strnicmp(data, pattern, dlen))
        {   
            return -1;
        }
        else
...
static int mangle_http_header(const struct sk_buff *skb, int flags)
{
    ...
    struct tcphdr *tcph = ipip_hdr(skb);
    ...
    {
        found = find_pattern2(data, datalen, "Cookie: ",sizeof("Cookie: ")-1, '\r', &offset, &len);
        if (found) 
        {
            char c;
...
static int get_http_info(const struct sk_buff *skb, int flags, httpinfo_t *info)
{
    ...
    SPARQ_LOG("%s: flags:0x%x seq=%u, datelen:[%d]\n",__FUNCTION__, flags, ntohl(tcph->seq), datalen);
...
{
    const struct ipt_webstr_info *info = (struct ipt_webstr_info *)(par->matchinfo);
...
match_ret:
    SPARQ_LOG("%s: Verdict =======> %s \n",__FUNCTION__, found ? "DROP" : "ACCEPT");
    return (((found ^ info->invert) > 0)? true : false);
}
