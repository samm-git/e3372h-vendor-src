...
char hdlc_cmd_val[][8] = {
      ...
      /*CMD_NAK_INVALID_FCS*/      {0x7E, 0x03, 0x00, 0x01, 0x21, 0x38, 0x7E},        /*CRCĞ£Ñé´íÎó*/ 
      ...
      /*CMD_NAK_EARLY_END*/        {0x7E, 0x03, 0x00, 0x04, 0x8C, 0x6F, 0x7E},        /*Êı¾İ³¤¶È¹ı¶Ì*/ 
      /*CMD_NAK_TOO_LARGE*/        {0x7E, 0x03, 0x00, 0x05, 0x05, 0x7D, 0x5E, 0x7E},  /*Êı¾İ³¤¶È¹ı³¤*/
      /*CMD_NAK_INVALID_CMD*/      {0x7E, 0x03, 0x00, 0x06, 0x9E, 0x4C, 0x7E},        /*ÎŞĞ§µÄÃüÁî*/ 
      /*CMD_NAK_FAILED*/           {0x7E, 0x03, 0x00, 0x07, 0x17, 0x5D, 0x7E},        /*²Ù×÷ÎŞ·¨Íê³É*/
      /*CMD_NAK_WRONG_IID*/        {0x7E, 0x03, 0x00, 0x08, 0xE0, 0xA5, 0x7E},        /*Î´Ê¹ÓÃ*/
      /*CMD_NAK_BAD_VPP*/          {0x7E, 0x03, 0x00, 0x09, 0x69, 0xB4, 0x7E},        /*Î´Ê¹ÓÃ*/
      /*CMD_NAK_VERIFY_FAILED*/    {0x7E, 0x03, 0x00, 0x0A, 0xF2, 0x86, 0x7E},        /*¼øÈ¨Ê§°Ü,Ã»ÓĞÈ¨ÏŞ²Ù×÷*/ 
      /*CMD_NAK_NO_SEC_CODE*/      {0x7E, 0x03, 0x00, 0x0B, 0x7B, 0x97, 0x7E},        /*Ã»ÓĞ°²È«Âë*/
      /*CMD_NAK_BAD_SEC_CODE*/     {0x7E, 0x03, 0x00, 0x0C, 0xC4, 0xE3, 0x7E},        /*°²È«Âë´íÎó/ÎŞĞ§*/
      /*CMD_Reserved*/             {0x7E, 0x03, 0x00, 0x0D, 0x4D, 0xF2, 0x7E},        /*±£Áô*/
      /*CMD_NAK_OP_NOT_PERMITTED*/ {0x7E, 0x03, 0x00, 0x0E, 0xD6, 0xC0, 0x7E},        /*ÏÖÌØĞÔ²»ÔÊĞí¸Ã²Ù×÷*/
      /*CMD_NAK_INVALID_ADDR*/     {0x7E, 0x03, 0x00, 0x0F, 0x5F, 0xD1, 0x7E},        /*´ËµØÖ·µÄÄÚ´æÎŞ·¨·ÃÎÊ*/                                     
      /*CMD_DLOAD_SWITCH*/         {0x7E, 0x3A, 0xA1, 0x6E, 0x7E},                    /*0x3A*/                
      /*CMD_CDROM_STATUS*/         {0x7E, 0xFB, 0x00, 0xE7, 0x97, 0x7E},              /*0xFB+0x00(ÎŞĞèÉ¾³ıCDROM)*/
                                   {0},
                                   {0}                                                                                                                                  
    };
/*****************************************************************************
...
void dcb_init( void)
{
    ...
    dcb->image_type                = IMAGE_PART_TOP;/*µ±Ç°Éı¼¶²¿¼şµÄÓ³ÏñÀàĞÍ*/
    dcb->image_size                = 0;             /*µ±Ç°Éı¼¶²¿¼şµÄ´óĞ¡*/
    dcb->flash_current_offset      = 0xFFFFFFFF;    /*µ±Ç°°üÔÚFLASHÖĞµÄÉÕĞ´µØÖ·,½öFLASHÓ³ÏñÏà¹Ø*/
    dcb->ram_addr                  = 0xFFFFFFFF;    /*µ±Ç°½ÓÊÕµ½µÄ°üÔÚRMAÖĞµÄÔ´µØÖ·*/     
    dcb->current_loaded_length     = 0;             /*µ±Ç°½ÓÊÕµ½µÄ°ü³¤¶È*/
    dcb->last_loaded_total_length  = 0;             /*ÉÏ´ÎÎªÖ¹ÒÑÊÕµ½µÄ°ü×Ü³¤¶È*/
    dcb->last_loaded_packet        = 0;             /*ÉÏ´ÎÒÑÊÕµ½µÄ°üĞòºÅ*/ 
...
{
//TODO:Ó²¼ş°æ±¾ºÅĞèÒª¸ù¾İsysfsÊµÏÖ
    return ERROR;
}
...
/*****************************************************************************
* º¯ Êı Ãû  : hdlc_send_cmd
*
* ¹¦ÄÜÃèÊö  : ÏòPC·¢ËÍHDLC»Ø¸´ÃüÁî
*
* ÊäÈë²ÎÊı  : enum dload_cmd  
* Êä³ö²ÎÊı  : 
*
* ·µ »Ø Öµ  : 
*
...
int hdlc_send_cmd(enum dload_cmd cmd)
{
    ...
    switch (cmd){
        ...
        /*µ¥°å°æ±¾IDĞÅÏ¢»Ø¸´,ĞèÓë½«ÏÂÔØµÄÓ³ÏñÎÄ¼şµÄÓ²¼ş°æ±¾ÍêÈ«Ò»ÖÂ,·ñÔò²»ÄÜÏÂÔØ*/
        case CMD_VERIFY:
        {
...
int pkt_process(DLOAD_HDLC_S *pkt)
{
    ...
    switch(cmd_recv){
        ...
        /*ÏÂÔØÄ£Ê½ÇĞ»»ĞéÄâÃüÁî,UE²àÎŞĞèÈÎºÎ¶¯×÷*/
        case CMD_DLOAD_SWITCH:
        {
            if( 1 != pkt->u32InfoLen )
            {
        ...
        /*ÓÃÓÚ¹¤¾ßÅĞ¶ÏÊÇ·ñÒÑÕıÈ·½øÈëDLOADÄ£Ê½*/
        case CMD_VERREQ:
        {
            if( 1 != pkt->u32InfoLen )
            {
            ...
            {                
                /*ÉèÖÃµ±Ç°¼ÓÔØ×´Ì¬ÎªUSB¼ÓÔØÖĞ,²¢µãµÆÖ¸Ê¾*/
        ...
        /*µ¥°åÈí¼ş°æ±¾ĞÅÏ¢²éÑ¯*//*½ö´æÔÚÓÚÇ¿ÖÆ¼ÓÔØÄ£Ê½*/
        case CMD_TCPU_VER: 
        {   
            if( 1 != pkt->u32InfoLen )
            {
        ...
        break;
        /*µ¥°åÈí¼ş°æ±¾ĞÅÏ¢²éÑ¯*//*½ö´æÔÚÓÚbootrom¶ş´ÎÖØÆôÓÅ»¯·½°¸ by 
m00176101 */
        case CMD_BCPU_VER: 
        {   
		...
		align_size = DLOAD_NAND_SPARE_ALIGN;
            ...
            /*ÔÚdload iso verison ÎÄ¼ş³õÊ¼»¯ÃüÁîÊ±£¬ÏÈ²Á³ıisoversion·ÖÇø*/ 
            ...
            /*Èç¹ûÉı¼¶µÄÊÇºóÌ¨£¬ÅĞ¶ÏºóÌ¨´óĞ¡ÊÇ·ñºÏÊÊ*/
            if(IMAGE_CDROMISO == dcb->image_type)
            {
                if(dcb->image_size > MAX_CDROMISO_SIZE)/*´óÓÚ88MB £¬²»Éı¼¶*/
                {
                    cmd_send = CMD_NAK_FAILED;
            ...
            {
               /*ÆÈÊ¹¹¤¾ß·¢ËÍµ¥°å¸´Î»ÃüÁî*/
                taskDelay(5);  /*¼ÓÈëÑÓ³Ù·ÀPC¿¨ËÀ:ÒÅÁôÎÊÌâ m00176101*/  
        ...
        { 
			...
			state_show(DLOAD_STATE_USB_RUNNING);
	            ...
	            else    /*(ulPacketNum == dcb->last_loaded_packet+1)Ë³Ğò°üÕı³£ÉÕĞ´*/
	            {
	                ;
	            }
              
		 /*»ñÈ¡µ±Ç°½ÓÊÕµ½µÄ°üÔÚRMAÖĞµÄÔ´µØÖ·*/
		dcb->ram_addr = (unsigned int)(pkt->au8DecapBuf+ 7); 
		if( IMAGE_CDROMISOVER == dcb->image_type )	
                {
			 /*½«ÏÂ·¢µÄISO version±£´æ£¬µÈISOÎÄ¼şÉı¼¶Íê³ÉºóÔÙĞ´ÈëÎÄ¼şÏµÍ³*/ 
	...
	break;
		...
		/*ÅĞ¶ÏÈç¹ûÎªrecovery¾µÏñ£¬ĞèÒªĞŞ¸Ä·ÖÇø±íÖĞµÄcountÖµ£¬ÒÔ±ãÖ§³ÖË«±¸·İ*/
		if(update_image_count(finish_image_type)){
			Dbug("Error: modify recovery count in ptalbe fail!\n");
			cmd_send = CMD_NAK_FAILED;
		}

            	/*½«ÉèÖÃNV»Ö¸´±ê¼ÇÎ»µÄ²Ù×÷ÒÆµ½Ò»¸ö²¿¼şÉı¼¶Íê³Éºó*/
            	/*µ±Éı¼¶µÄ×é¼şÓĞÒ»¸ö²»ÎªºóÌ¨ÎÄ¼şµÄÊ±ºò£¬Æô¶¯ºócdrom²»¼ÓÔØ£¬½øÈënv»Ö¸´Á÷³Ì*/
		if((IMAGE_CDROMISOVER != dcb->image_type)\
		...
		{
			/*ÅĞ¶ÏÈç¹ûÎªptable¾µÏñ£¬ĞèÒªĞŞ¸Ä·ÖÇø±íÖĞµÄcountÖµ£¬ÒÔ±ãÖ§³ÖË«±¸·İ*/
	   ...
	   printf("\nimagetype:%xdload_seccuss!\n",dcb->image_type);
           dcb->finish_image_type = IMAGE_PART_TOP;
            /*±¾Ó³ÏñÕı³£ÉÕĞ´Íê±Ï£¬½«È«¾Ö±äÁ¿¸´Î»*/
            write_buf.used = 0;    /*Çå¿Õg_stDloadWriteBuffer ,²»ĞèÒªÊÍ·ÅÃ´å*/ 
            dcb_init();    /*Çå¿ÕDCB±í*/
...
int dload_main(void)
{
    	...
    	while(1)
	{
        	...
        	if(ATCOMMAND_PROCESS_SUCEED == at_cmd_proc(recv_buf,read_len))
		{
        	...
        	/* Êı¾İ°ü´¦Àí */        
        	/*µ÷ÓÃHDLC½Ó¿Ú£¬½«pBufÖĞµÄÊı¾İ½âÎöµ½pkt->au8DecapBuf*/
        	for ( i = 0; i < read_len; i++ )
        	{
            		...
            		if ( HDLC_OK == ret )
            		{ 
                		/*Ò»Ö¡Êı¾İ°ü½ÓÊÕÍê³É*//*HDLC½â·â×°º¯ÊıÍ¨¹ı0x7E±êÖ¾·û·ÖÖ¡*/     
                		pkt_process(hdlcpkt);
            		}
