...
* Description: nandc_host的相关数据结构的操作都放在此文件中
*
*******************************************************************************/
/******************************************************************************
...
u32  nandc_host_set_partition(struct nandc_host * host, struct mtd_partition* ptable, u32 nr_parts)
{
    ...
    if(totalchip > NANDC_MAX_CHIPS)
    {
        NANDC_DO_ASSERT(0 ,"totalchip max error", totalchip) ;
        return NANDC_ERROR;
    }
	/*如果ptable是不存的话*/
    if(NANDC_NULL == ptable)
    {
       ...
       {
            if(NANDC_OK == nandc_host_get_ecc_ability(host, p_nand[curchip],  &maxecc))/*[false alarm]:屏蔽fortify错误 */
            {
    ...
    else
    {
       ...
       for (curchip = 0; curchip < totalchip; curchip++)
       {
            sizemax += p_nand[curchip]->spec.chipsize;/*[false alarm]:屏蔽fortify错误 */
            if(NANDC_OK == nandc_host_get_ecc_ability(host, p_nand[curchip],  &maxecc))/*[false alarm]:屏蔽fortify错误 */
            {
                if(host->ecctype_onfi)
                {
