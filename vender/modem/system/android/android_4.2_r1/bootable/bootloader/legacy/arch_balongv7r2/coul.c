...
unsigned int coul_cali_voltage(unsigned voltage)
{
    ...
    calied_volt = coul_cali_config.v_offset_a *voltage;
...
void bsp_coul_eco_filter_time(COUL_FILETR_TIME filter_time)
{
    ...
    coul_reg_mask(HI6551_CLJ_CTRL_REG_OFFSET,filter_time,COUL_CTRL_FILETR_MASK);
...
/*****************************************************************************
 函 数 名  : bsp_coul_is_normal
 功能描述  : 查询库仑计是否进入normal态
 输入参数  : void
 输出参数  : 无
 返 回 值  : 1:normal态；0:未到normal态
 调用函数  :
 被调函数  :
*****************************************************************************/
bool bsp_coul_is_normal(void)
{
    unsigned char regval = 0;

    regval = coul_reg_read(HI6551_STATE_TEST_OFFSET);

    regval = regval & COUL_STATE_MASK;

    return  (regval > 0x3) ? 1 : 0;
}
/*****************************************************************************
