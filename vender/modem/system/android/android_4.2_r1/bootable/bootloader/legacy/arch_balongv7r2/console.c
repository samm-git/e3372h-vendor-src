...
void uart_sele_set(char c)
{
	...
	case 'L':
		...
		/*shell*/
		if(HW_VER_PRODUCT_E5379 == bsp_version_get_board_type())
		{
...
void console_rx_cmd_complete(char* cmd)
{
	...
	else if(memcmp(cmd, "gpio_expander", 13) == 0){
		gpio_ex_show();
	}
	...
	else if(memcmp(cmd, "m3_pm_test", 10) == 0){
		boot_rtx_cm3_from_flash();
        		/*lint -save -e716*/
		        /* coverity[no_escape] */
                while(1)
...
void console_poll()
{
	...
	if (console_buf_loc + 2 == MAX_CONSOLE_BUF
		|| ch == '\r'
		|| ch == '\n') {
		console_buf[console_buf_loc++] = (char)ch;
		console_buf[console_buf_loc] = 0;
