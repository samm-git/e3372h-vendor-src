...
int Cipher_StartKdfChannel()
{
	...
	unsigned int delay_loops = 20000;/*循环20000次，每次延迟1us，共延迟20000us*/
...
void cipher_open_clk(void)
{
	unsigned int reg_val = 0;

	//set cipher clk divivor
	CIPHER_REG_READ(HI_SYSCRG_BASE_ADDR_VIRT, HI_CRG_CLKDIV3_OFFSET, reg_val);
	reg_val &= 0xFFFFFFF;//clear 28-31bits
	reg_val |= (5U << 28);//set cipher divivor to 5, the default divivor
	CIPHER_REG_WRITE(HI_SYSCRG_BASE_ADDR_VIRT, HI_CRG_CLKDIV3_OFFSET, reg_val);
	//open cipher clk
	CIPHER_REG_WRITE(HI_SYSCRG_BASE_ADDR_VIRT, HI_CRG_CLKEN4_OFFSET, 1U << 17);
	//wait for clk being open
	do
	{
		CIPHER_REG_READ(HI_SYSCRG_BASE_ADDR_VIRT, HI_CRG_CLKSTAT4_OFFSET, reg_val);
	}while(!(reg_val & (1U << 17)));
}
void cipher_close_clk(void)
{
	CIPHER_REG_WRITE(HI_SYSCRG_BASE_ADDR_VIRT, HI_CRG_CLKDIS4_OFFSET, 1U << 17);
}
	...
	unsigned long * out_hash, unsigned long * out_hash_len)
{
	...
	unsigned char BD_RD_BUFF[CIPHER_ALIGNE_LEN + BDQ_SIZE + RDQ_SIZE +1] = {0};
	...
	int i = 0;
	unsigned int bdq_base_addr = 0;
	unsigned int rdq_base_addr = 0;
	...
	*(key_get.penOutKeyLen) = 0;
	...
	bdq_base_addr = ((unsigned)BD_RD_BUFF + CIPHER_ALIGNE_LEN) & ~CIPHER_ALIGNE_LEN;
	rdq_base_addr = bdq_base_addr + BDQ_SIZE;
	CIPHER_Init((void*)bdq_base_addr, (void*)rdq_base_addr);
