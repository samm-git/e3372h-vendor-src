...
XML_RESULT_ENUM_UINT32 xml_write_nv_data(void)
{
    ...
    returnval = nv_boot_search_byid(usnvitemid,((u8*)NV_GLOBAL_CTRL_INFO_ADDR),&ref_info,&file_info);
    if(returnval)
    {
        return XML_RESULT_SUCCEED;
    }
    ...
    returnval = nv_boot_write_to_mem(xml_ctrl.g_puclnvitem,ref_info.nv_len,file_info.file_id,ref_info.nv_off);
    if(returnval)
    {
        xml_write_error_log(__LINE__, usnvitemid, returnval);
        goto out;
...
XML_RESULT_ENUM_UINT32 xml_decode_xml_file(FILE* pfile)
{
    ...
    /*read 4k*/
    lreaded = nv_emmc_read((u8*)(xml_ctrl.g_pclfilereadbuff),1,XML_FILE_READ_BUFF_SIZE,pfile);
    ...
    while(0 != lreaded)
    {
        for(lcount = 0;lcount < lreaded; lcount++)
        {
               ...
               (XML_DECODE_STATUS_JUMP == xml_ctrl.g_stlxmljumpflag))
            {
                ...
                nv_emmc_seek(pfile,(s32)(xml_ctrl.g_stlxmljumpinfo.productCatOff),SEEK_SET);
               ...
               (XML_DECODE_STATUS_JUMP == xml_ctrl.g_stlxmljumpflag))
            {
                ...
                nv_emmc_seek(pfile,(s32)(xml_ctrl.g_stlxmljumpinfo.productIdOff),SEEK_SET);
...
void xml_productid_format(int product_id)
{
    int i = 0;
    int len = 0;
    s8 temp;
...
XML_RESULT_ENUM_UINT32 xml_getproductid(void)
{
	...
	/* 取得当前单板的Product id */
	lproductid=  bsp_version_get_boardid();/*get hardware version*/
