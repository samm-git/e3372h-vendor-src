...
const struct nandc_ecc_info nandc_fix_onchiprom_bug_eccinfo[] =
{
    {NANDC_PAGE_SIZE_4K_REG,     NANDC_SPARE_LENGTH_144BYTE_REG,    NANDC_ECCTYPE_ECC_8BIT_REG,NULL},
    {NANDC_PAGE_SIZE_4K_REG,     NANDC_SPARE_LENGTH_88BYTE_REG,     NANDC_ECCTYPE_ECC_4BIT_REG,NULL},

    {NANDC_PAGE_SIZE_2K_REG,     NANDC_SPARE_LENGTH_88BYTE_REG,     NANDC_ECCTYPE_ECC_8BIT_REG,NULL},
    {NANDC_PAGE_SIZE_2K_REG,     NANDC_SPARE_LENGTH_60BYTE_REG,     NANDC_ECCTYPE_ECC_4BIT_REG,NULL},

    {0,0,0,NULL},
};

void nandc_fix_oobsize_from_pagesize_ecctype(struct nand_spec_sram *spec_real)
{
    const struct nandc_ecc_info* ecc_table = nandc_fix_onchiprom_bug_eccinfo;
    u32 ecctype = spec_real->ecc_type;
    u32 pagesize = spec_real->page_size;

    while(0 != ecc_table->pagesize)
    {
        if((pagesize == ecc_table->pagesize ) && (ecctype == ecc_table->ecctype))
        {
            spec_real->spare_size = ecc_table->sparesize;
            return;
        }
        ecc_table++;
    }
...
/*onchiprom中有一种flash的参数得到不全,需要再来设置一下*/
void nandc_fix_get_param_from_onchiprom_bug(struct nand_spec_sram *nandspec)
{
    if((nandspec->flag == NAND_ARGS_PART_AVAIL) && (nandspec->chip_size) && (nandspec->spare_size ==0))
    {
        nandc_fix_oobsize_from_pagesize_ecctype(nandspec);
        nandspec->flag = NAND_ARGS_FULL_AVAIL;
...
u32 nandc_native_nand_prob(struct nandc_host* host)
{
    ...
    u32 addnum = 0;
    int ret=0;
    tOcrShareData *pShareData = (tOcrShareData*)SRAM_SEC_SHARE;
    ...
    if(is_usb_burn_trapflag())
    {
        ...
        nandc_fix_get_param_from_onchiprom_bug(&(pShareData->nandspec));
    }
    else
    {
        nandc_fix_get_param_from_onchiprom_bug(&(pShareData->nandspec));
