...
* Version: v1.0
*
* Filename:    nandc_v600.c
* Description: nandc_v600中的硬件寄存器的定义
*******************************************************************************/
/******************************************************************************
...
/*nandc_v600的硬件寄存器的定义，这里是每一个寄存器的定义，每一个寄存器的定义中又来按bit位来定义功能*/
...
/*nandc_v600在ECC24的方式下面的数据格式*/
struct nandc_dmap datamap_ecc24_v6  =
{
    STU_SET(.bad2k) 2048,
    STU_SET(.pmap2k)
    {
...
/*nandc_v600在不同的ECC方式下面的硬件缓冲器的数据格式*/
...
/*nandc_v600的初始化配置信息,后面的nandc_v600的硬件寄存器的初始化就以此变量来初始化*/
