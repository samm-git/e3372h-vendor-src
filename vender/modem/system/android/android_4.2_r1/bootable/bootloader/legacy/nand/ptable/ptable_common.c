...
u32 ptable_copy_from_ptable_product(void)
{
...
u32 ptable_parse_mtd_partitions(struct mtd_partition** mtd_parts, u32 *nr_parts)
{
    ...
    /*lint -save -e539*/
    /*lint -restore*/
    ptable_adapt();   /*恢复最后一个分区自适应功能(mbb)*/
...
#if (FEATURE_ON == MBB_COMMON)
    /*额外增加几个空的分区，主要是在升级过程中（没有二次重启），分区表增多时，不能够找到对应的新增节点，造成升级失败故障*/
    while(npart < MAX_PARTITIONS_NUM)
    {
        parts->name   = "nullMTD";
        parts->offset = 0;
        parts->size   = -1; /*设置成-1表示该分区大小是0*/
        parts++;
        npart++;
    }
    *nr_parts  = MAX_PARTITIONS_NUM;
