...
*   作    者 :  x00195528
*
*   描    述 :  PMU设备驱动程序，主要功能是控制PMIC（hi6451或hi6559）
*
*
*   问题单号                 修改人             修改内容
                                               多台加载升级到50%中断，重启后单板进入升级模式，只有电源灯单闪
*************************************************************************/
#include <boot/boot.h>
...
/*启动时配置pmu欠压警告阈值为2700mV*/
static __inline__ void hi6559_uvp_threshold_set_2700(void)      /*lint !e129*/
{
...
void hi6559_optreg_config(void)
{
   ...
   /*根据硬件要求，提高buck0的ocp点，防止apt模式下buck0电压跳变上报过流中断*/
   bsp_hi6559_reg_write(HI6559_BUCK0_REG15_ADJ_OFFSET, 0xf);
   ...
   /* buck5 */
   bsp_hi6559_reg_write(HI6559_BUCK5_REG5_ADJ_OFFSET, 0x30);
   bsp_hi6559_reg_write(HI6559_BUCK5_REG9_ADJ_OFFSET, 0x04);
   ...
   bsp_hi6559_reg_write(HI6559_BUCK6_REG7_ADJ_OFFSET, 0x01);    /* 减小最小导通时间 */
   bsp_hi6559_reg_write(HI6559_BUCK6_REG9_ADJ_OFFSET, 0x07);    /* 增加regulator调节范围 */

   /* 规避buck0过流问题.初始化时0x20寄存器配置成0x65，BUCK OCP/SCP上升沿去抖500us */
   bsp_hi6559_reg_write(HI6559_OCP_DEB_OFFSET, 0x65);
}
...
void hi6559_volt_config(void)
{
    ...
    ret = bsp_hi6559_volt_set_voltage(PMIC_HI6559_LDO24,3000000,3000000,&selector);
    ret |= bsp_hi6559_volt_enable(PMIC_HI6559_LDO24);
    
    /* 配置hi6559 LDO1 2.5v on， LDO1要在LDO24之后打开 */
    ret |= bsp_hi6559_volt_set_voltage(PMIC_HI6559_LDO01,2500000,2500000,&selector);
    ret |= bsp_hi6559_volt_enable(PMIC_HI6559_LDO01);

#if defined(BSP_CONFIG_BOARD_E5)
#if (FEATURE_ON == MBB_LED)
    /* 配置hi6559 LDO11 3.3v on */
    ret |= bsp_hi6559_volt_set_voltage(PMIC_HI6559_LDO11,3300000,3300000,&selector);
    ret |= bsp_hi6559_volt_enable(PMIC_HI6559_LDO11);
#endif
#endif

    if(ret)
    {
        pmic_print_error("\r\npmic config error, ret = %d!", ret);
    }
}
...
void pmu_hi6559_init(void)
{
	...
	*volt_need_off = 0;
    ...
    iregVal = 0x07;
    bsp_hi6559_reg_write(HI6559_BUCK5_REG8_ADJ_OFFSET, iregVal);
#endif
...
s32 hi6559_get_boot_state(void)
{
    ...
    s32 flag = 0;
    u32 j = 0;
    u8 hi6559_om_boot[HI6559_NP_RECORD_REG_NUM] = {0};
    ...
    /* 保存寄存器到共享内存 */
    memcpy((void *)SHM_MEM_PMU_NPREG_ADDR, (const void *)hi6559_om_boot, HI6559_NP_RECORD_REG_NUM);
