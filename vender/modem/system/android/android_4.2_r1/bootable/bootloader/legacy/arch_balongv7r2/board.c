...
void board_init(void)
{
    ...
    {
        (void)ptable_copy_from_ptable_product();
    }
    (void)flash_init();
...
#if (defined (CONFIG_FB_EMI_BALONG) || defined (CONFIG_FB_SPI_BALONG) || defined (CONFIG_FB_0_9_6_INCH_BALONG))
...
void board_getvar(const char *name, char *value)
{
     ...
     }else if(!memcmp(name, "pagesize", 8))
     {
        struct nand_spec spec;  
        
        cprintf("Try to get pagesize...\n");
        if(bsp_get_nand_info(&spec))
        {
            dprintf("Get flash info failed\n");
            return;
        }
        
        dprintf("pagesize=%d\n", spec.pagesize);
        snprintf(value, 32, "%d", spec.pagesize);        
     }
