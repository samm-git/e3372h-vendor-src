...
/**
* 作用:nandc模块提供给usbloader.c的接口,可以把有ecc error的数据写入到nandflash中方便复现现象
*
* 参数:
* @flash_addr             ---Flash地址
* @src_addr               ---存放数据的源地址
...
* 描述:主要是给在pc机上通过fastboot.exe来保存一整页的数据
*/
u32 nand_write_rawpage_usbloader(FSZ flash_addr, u32 src_data, u32 write_size)
{
    struct nandc_host* host = nandc_nand_host ;
    struct nandc_spec* spec;
    u32 ret = NANDC_ERROR;
    u32 pageoffset=0;
    u32 pageonumber=0;
    u32 whole_rawpagesize=0;

    spec = &host->nandchip->spec ;
    pageoffset  =   (u32)(flash_addr & spec->offinpage);
    pageonumber =   (u32)(flash_addr >>spec->pageshift);
    whole_rawpagesize = spec->pagesize+spec->sparesize;
    if((pageoffset != 0 ) ||(write_size != whole_rawpagesize))
    {
        /* coverity[printf_arg_mismatch] */
        NANDC_TRACE(NFCDBGLVL(ERRO), ("write raw error!! flash_addr 0x%x,read_size 0x%x \n",flash_addr,write_size));
        goto EXIT;
    }
    NANDC_TRACE(NFCDBGLVL(ERRO), ("flash_addr 0x%x,read_size 0x%x pageonumber %x whole_rawpagesize %x \n",flash_addr,write_size,pageonumber,whole_rawpagesize));
    NANDC_TRACE(NFCDBGLVL(ERRO), ("nandc_nand_write_raw data 0x%x\n",*((int *)src_data)));
    ret = nandc_nand_write_raw(pageonumber,pageoffset,(u8*)src_data,write_size);
    NANDC_TRACE(NFCDBGLVL(ERRO), ("nandc_nand_write_raw 0x%x\n",ret));
