 ...
 ******************************************************************************
  ...
  1.日    期   : 2012年12月6日
    作    者   : s45755
    修改内容   : 从《Hi6620V100 BBP寄存器手册_WCDMA_ON.xml》自动生成
...
#define WBBP_CK_MSR_FRAC_ADDR                         (SOC_BBP_WCDMA_ON_BASE_ADDR + 0x3C08)

/* 寄存器说明：
 bit[31:11]  保留
 bit[10:0]   时钟测量整数部分，即ck_msr_counter除以ck_msr_period的整数部分。
   UNION结构:  WBBP_CK_MSR_INTEGER_UNION */
#define WBBP_CK_MSR_INTEGER_ADDR                      (SOC_BBP_WCDMA_ON_BASE_ADDR + 0x3C0C)

/* 寄存器说明：
 bit[31:12]  保留
 bit[11:0]   产生深睡指示的10ms帧数，即表示在唤醒之前多长时间内，BBP产生深睡指示。单位，10ms。
   UNION结构:  WBBP_DEEP_SLEEP_THRESHOLD_UNION */
#define WBBP_DEEP_SLEEP_THRESHOLD_ADDR                (SOC_BBP_WCDMA_ON_BASE_ADDR + 0x3C2C)
...
#define WBBP_DEEP_SLEEP_IND_1CARD_ADDR                (SOC_BBP_WCDMA_ON_BASE_ADDR + 0x3C30)

/* 寄存器说明：
 bit[31:1]   保留
 bit[0]      BBP唤醒方式。1，表示CPU强制唤醒；0，表示BBP根据配置时刻唤醒。
   UNION结构:  WBBP_BBP_WAKEUP_TYPE_2CARD_UNION */
#define WBBP_BBP_WAKEUP_TYPE_2CARD_ADDR               (SOC_BBP_WCDMA_ON_BASE_ADDR + 0x3C48)

/* 寄存器说明：
 bit[31:1]   保留
 bit[0]      BBP睡眠查询标志。1，表示BBP处于睡眠状态；0，表示BBP处于唤醒状态。
   UNION结构:  WBBP_BBP_SLEEP_STATUS_2CARD_UNION */
#define WBBP_BBP_SLEEP_STATUS_2CARD_ADDR              (SOC_BBP_WCDMA_ON_BASE_ADDR + 0x3C4C)

/* 寄存器说明：
 bit[31:1]   保留
 bit[0]      上报ARM的深睡指示。1，表示此时距离唤醒时刻的时间小于deep_sleep_threshold配置的时间；0，表示此时距离唤醒时刻的时间大于deep_sleep_threshold配置的时间。
   UNION结构:  WBBP_DEEP_SLEEP_IND_2CARD_UNION */
#define WBBP_DEEP_SLEEP_IND_2CARD_ADDR                (SOC_BBP_WCDMA_ON_BASE_ADDR + 0x3C50)
...
#define WBBP_SYS_CNT_SLEEP_2CARD_ADDR                 (SOC_BBP_WCDMA_ON_BASE_ADDR + 0x3C54)
...
#define WBBP_SYS_CNT_SLEEP_1CARD_ADDR                 (SOC_BBP_WCDMA_ON_BASE_ADDR + 0x3C64)

/* 寄存器说明：
 bit[31:1]   保留
 bit[0]      BBP唤醒方式。1，表示CPU强制唤醒；0，表示BBP根据配置时刻唤醒。
   UNION结构:  WBBP_BBP_WAKEUP_TYPE_1CARD_UNION */
#define WBBP_BBP_WAKEUP_TYPE_1CARD_ADDR               (SOC_BBP_WCDMA_ON_BASE_ADDR + 0x3C70)

/* 寄存器说明：
 bit[31:1]   保留
 bit[0]      BBP睡眠查询标志。1，表示BBP处于睡眠状态；0，表示BBP处于唤醒状态。
   UNION结构:  WBBP_BBP_SLEEP_STATUS_1CARD_UNION */
#define WBBP_BBP_SLEEP_STATUS_1CARD_ADDR              (SOC_BBP_WCDMA_ON_BASE_ADDR + 0x3C74)

/* 寄存器说明：
 bit[31:2]   保留
 bit[1:0]    中断状态寄存器。1，表示相应比特位置有中断；0，表示相应比特位置没有中断。
   UNION结构:  WBBP_ARM_INT02_STA_UNION */
#define WBBP_ARM_INT02_STA_ADDR                       (SOC_BBP_WCDMA_ON_BASE_ADDR + 0x3C80)

/* 寄存器说明：
 bit[31:1]   保留
 bit[0]      中断使能寄存器。1，表示使能中断；0，表示不使能中断。
   UNION结构:  WBBP_ARM_INT02_EN_UNION */
#define WBBP_ARM_INT02_EN_ADDR                        (SOC_BBP_WCDMA_ON_BASE_ADDR + 0x3C88)

/* 寄存器说明：
 bit[31:4]   保留
 bit[3:0]    中断状态寄存器。1，表示相应比特位置有中断；0，表示相应比特位置没有中断。
   UNION结构:  WBBP_ARM_INT03_STA_UNION */
#define WBBP_ARM_INT03_STA_ADDR                       (SOC_BBP_WCDMA_ON_BASE_ADDR + 0x3C90)

/* 寄存器说明：
 bit[31:1]   保留
 bit[0]      中断使能寄存器。1，表示使能中断；0，表示不使能中断。
   UNION结构:  WBBP_ARM_INT03_EN_UNION */
#define WBBP_ARM_INT03_EN_ADDR                        (SOC_BBP_WCDMA_ON_BASE_ADDR + 0x3C98)
...
#define WBBP_DRX_RST_CON_ADDR                         (SOC_BBP_WCDMA_ON_BASE_ADDR + 0x3CC0)
...
#define WBBP_DRX_AFC_VALUE_ADDR                       (SOC_BBP_WCDMA_ON_BASE_ADDR + 0x3CC4)
...
#define WBBP_SLEEP_EN_1CARD_ADDR                      (SOC_BBP_WCDMA_ON_BASE_ADDR + 0x3C00)

/* 寄存器说明：
 bit[31:1]   保留
 bit[0]      CPU强制唤醒使能。1，表示BBP立即唤醒；0，无效。BBP内部将此信号处理为脉冲信号。
   UNION结构:  WBBP_SWITCH_UP_1CARD_UNION */
#define WBBP_SWITCH_UP_1CARD_ADDR                     (SOC_BBP_WCDMA_ON_BASE_ADDR + 0x3C04)

/* 寄存器说明：
 bit[31:12]  保留
 bit[11:0]   从唤醒时刻到时钟切换时刻32.768k时钟个数。
   UNION结构:  WBBP_CK_SWITCH_CNT_1CARD_UNION */
#define WBBP_CK_SWITCH_CNT_1CARD_ADDR                 (SOC_BBP_WCDMA_ON_BASE_ADDR + 0x3C14)

/* 寄存器说明：
 bit[31:12]  保留
 bit[11:0]   唤醒时刻SFN值。
   UNION结构:  WBBP_CK_WAKE_SFN_1CARD_UNION */
#define WBBP_CK_WAKE_SFN_1CARD_ADDR                   (SOC_BBP_WCDMA_ON_BASE_ADDR + 0x3C18)
...
#define WBBP_CK_WAKE_CNT_1CARD_ADDR                   (SOC_BBP_WCDMA_ON_BASE_ADDR + 0x3C1C)

/* 寄存器说明：
 bit[31:1]   保留
 bit[0]      中断屏蔽寄存器。1，表示屏蔽相应比特位置的中断；0，表示不屏蔽相应比特位置的中断。
   UNION结构:  WBBP_ARM_INT02_MASK_1CARD_UNION */
#define WBBP_ARM_INT02_MASK_1CARD_ADDR                (SOC_BBP_WCDMA_ON_BASE_ADDR + 0x3C84)

/* 寄存器说明：
 bit[31:1]   保留
 bit[0]      中断清除寄存器。1，表示清除相应比特位置中断；0，表示不清除相应比特位置的中断。
   UNION结构:  WBBP_ARM_INT02_CLR_1CARD_UNION */
#define WBBP_ARM_INT02_CLR_1CARD_ADDR                 (SOC_BBP_WCDMA_ON_BASE_ADDR + 0x3C8C)

/* 寄存器说明：
 bit[31:2]   保留
 bit[1:0]    中断屏蔽寄存器。1，表示屏蔽相应比特位置的中断；0，表示不屏蔽相应比特位置的中断。
   UNION结构:  WBBP_ARM_INT03_MASK_1CARD_UNION */
#define WBBP_ARM_INT03_MASK_1CARD_ADDR                (SOC_BBP_WCDMA_ON_BASE_ADDR + 0x3C94)

/* 寄存器说明：
 bit[31:2]   保留
 bit[1:0]    中断清除寄存器。1，表示清除相应比特位置中断；0，表示不清除相应比特位置的中断。
   UNION结构:  WBBP_ARM_INT03_CLR_1CARD_UNION */
#define WBBP_ARM_INT03_CLR_1CARD_ADDR                 (SOC_BBP_WCDMA_ON_BASE_ADDR + 0x3C9C)
...
#define WBBP_SLEEP_EN_2CARD_ADDR                      (SOC_BBP_WCDMA_ON_BASE_ADDR + 0x3C34)

/* 寄存器说明：
 bit[31:1]   保留
 bit[0]      CPU强制唤醒使能。1，表示BBP立即唤醒；0，无效。BBP内部将此信号处理为脉冲信号。
   UNION结构:  WBBP_SWITCH_UP_2CARD_UNION */
#define WBBP_SWITCH_UP_2CARD_ADDR                     (SOC_BBP_WCDMA_ON_BASE_ADDR + 0x3C38)

/* 寄存器说明：
 bit[31:12]  保留
 bit[11:0]   从唤醒时刻到时钟切换时刻32.768k时钟个数。
   UNION结构:  WBBP_CK_SWITCH_CNT_2CARD_UNION */
#define WBBP_CK_SWITCH_CNT_2CARD_ADDR                 (SOC_BBP_WCDMA_ON_BASE_ADDR + 0x3C3C)

/* 寄存器说明：
 bit[31:12]  保留
 bit[11:0]   唤醒时刻SFN值。
   UNION结构:  WBBP_CK_WAKE_SFN_2CARD_UNION */
#define WBBP_CK_WAKE_SFN_2CARD_ADDR                   (SOC_BBP_WCDMA_ON_BASE_ADDR + 0x3C40)
...
#define WBBP_CK_WAKE_CNT_2CARD_ADDR                   (SOC_BBP_WCDMA_ON_BASE_ADDR + 0x3C44)
...
#define WBBP_ARM_INT02_MASK_2CARD_ADDR                (SOC_BBP_WCDMA_ON_BASE_ADDR + 0x3CA0)
...
#define WBBP_ARM_INT02_CLR_2CARD_ADDR                 (SOC_BBP_WCDMA_ON_BASE_ADDR + 0x3CA4)
...
#define WBBP_ARM_INT03_MASK_2CARD_ADDR                (SOC_BBP_WCDMA_ON_BASE_ADDR + 0x3CA8)
...
#define WBBP_ARM_INT03_CLR_2CARD_ADDR                 (SOC_BBP_WCDMA_ON_BASE_ADDR + 0x3CAC)
