 ...
 ******************************************************************************
  ...
  版 本 号   : 初稿
  作    者   : Excel2Code
  生成日期   : 2013-07-30 16:24:43
  最近修改   :
  功能描述   : 接口头文件
  函数列表   :
  修改历史   :
  1.日    期   : 2013年7月30日
    修改内容   : 从《Hi6620V100 BBP寄存器手册_COMM.xml》自动生成
...
/* 寄存器说明：从模0对应通道的指示寄存器
 bit[31:1]   保留
 bit[0:0]    从模0对应的通道选择信号。
             0：选择通道0；
             1：选择通道1。
             配置时刻：配置时刻：各模根据通信场景配置
             
             V7R2删除。
   UNION结构:  BBPCOMM_C0_CH_NUM_IND_UNION */
#define BBPCOMM_C0_CH_NUM_IND_ADDR                    (SOC_BBP_COMM_BASE_ADDR + 0x8)
...
/* 寄存器说明：寄存器
 ...
 bit[2:0]    当前通道0（交叉之前）输出的强配模式。
             注：其适用于各个外围器件（ABB、RF、PA等）公共的交叉强配。
             调试用。
...
#define BBPCOMM_CPU_MIPI0_CLR_IND_IMI_ADDR            (SOC_BBP_COMM_BASE_ADDR + 0x114)
...
/* 寄存器说明：从模1对应通道的指示寄存器
 bit[31:1]   保留
 bit[0:0]    从模1对应的通道选择信号。
             0：选择通道0；
             1：选择通道1。
             配置时刻：各模根据通信场景配置
             
             V7R2删除。
   UNION结构:  BBPCOMM_C1_CH_NUM_IND_UNION */
#define BBPCOMM_C1_CH_NUM_IND_ADDR                    (SOC_BBP_COMM_BASE_ADDR + 0x308)
...
/* 寄存器说明：寄存器
   详      述：由软件直接控制MIPI1 master,不借助xbbp发送写指令时,写指令低32bit
   UNION结构 ：无 */
...
/* 寄存器说明：寄存器
 bit[31:1]   保留
 bit[0]      由软件直接控制MIPI1 master,需首先拉高此指示,屏蔽xbbp对MIPI0的控制
...
#define BBPCOMM_CPU_MIPI1_CLR_IND_IMI_ADDR            (SOC_BBP_COMM_BASE_ADDR + 0x414)
...
/* 寄存器说明：
 bit[31:1]   保留
 bit[0]      0:lte的SSI/MIPI控制字走SSI0/MIPI0
             1:lte的SSI/MIPI控制字走SSI1/MIPI1
             
             V7R2删除。
   UNION结构:  BBPCOMM_L_CH_SWITCH_UNION */
#define BBPCOMM_L_CH_SWITCH_ADDR                      (SOC_BBP_COMM_BASE_ADDR + 0x600)

/* 寄存器说明：
 bit[31:1]   保留
 bit[0]      0:w的SSI/MIPI控制字走SSI0/MIPI0
             1:w的SSI/MIPI控制字走SSI1/MIPI1
             
             V7R2删除。
   UNION结构:  BBPCOMM_W_CH_SWITCH_UNION */
#define BBPCOMM_W_CH_SWITCH_ADDR                      (SOC_BBP_COMM_BASE_ADDR + 0x604)

/* 寄存器说明：
 bit[31:1]   保留
 bit[0]      0:tds的SSI/MIPI控制字走SSI0/MIPI0
             1:tds的SSI/MIPI控制字走SSI1/MIPI1
             
             V7R2删除。
   UNION结构:  BBPCOMM_T_CH_SWITCH_UNION */
#define BBPCOMM_T_CH_SWITCH_ADDR                      (SOC_BBP_COMM_BASE_ADDR + 0x608)

/* 寄存器说明：
 bit[31:1]   保留
 bit[0]      0:g1的SSI/MIPI控制字走SSI0/MIPI0
             1:g1的SSI/MIPI控制字走SSI1/MIPI1
             
             V7R2删除。
   UNION结构:  BBPCOMM_G1_CH_SWITCH_UNION */
#define BBPCOMM_G1_CH_SWITCH_ADDR                     (SOC_BBP_COMM_BASE_ADDR + 0x60C)

/* 寄存器说明：定时锁存使能指示寄存器
 bit[31:1]   保留
 bit[0]      逻辑自清零。DSP需要获取定时关系时配置生效，CTU逻辑根据此信号锁存LTE/W/Gm/Gs/TDS的系统定时，CTU的内部定时供DSP查询。
             配置时刻：软件需要锁存定时
             生效时刻：立即生效
...
/* 寄存器说明：定时标志清除寄存器
 bit[31:1]   保留
 bit[0]      逻辑自清零。定时标志清除，用于清除timing_valid标志。
             配置时刻：DSP读取定时完毕后配置
             生效时刻：立即生效
...
/* 寄存器说明：寄存器
   详      述：W/TD/GM/GS为主模时写此寄存器，L为从模时读取此寄存器。此寄存器由DSP之间进行读写，BBP仅提供接口。
   UNION结构 ：无 */
#define BBPCOMM_MEAS_FLAG_L_RD_ADDR                   (SOC_BBP_COMM_BASE_ADDR + 0x640)

/* 寄存器说明：寄存器
   详      述：L/TD/GM/GS为主模时写此寄存器，W为从模时读取此寄存器。此寄存器由DSP之间进行读写，BBP仅提供接口。
   UNION结构 ：无 */
#define BBPCOMM_MEAS_FLAG_W_RD_ADDR                   (SOC_BBP_COMM_BASE_ADDR + 0x644)

/* 寄存器说明：寄存器
   详      述：W/L/GM/GS为主模时写此寄存器，TD为从模时读取此寄存器。此寄存器由DSP之间进行读写，BBP仅提供接口。
   UNION结构 ：无 */
#define BBPCOMM_MEAS_FLAG_T_RD_ADDR                   (SOC_BBP_COMM_BASE_ADDR + 0x648)

/* 寄存器说明：寄存器
   详      述：W/TD/L/GS为主模时写此寄存器，GM为从模时读取此寄存器。此寄存器由DSP之间进行读写，BBP仅提供接口。
   UNION结构 ：无 */
#define BBPCOMM_MEAS_FLAG_GM_RD_ADDR                  (SOC_BBP_COMM_BASE_ADDR + 0x64C)

/* 寄存器说明：寄存器
   详      述：W/TD/GM/L为主模时写此寄存器，GS为从模时读取此寄存器。此寄存器由DSP之间进行读写，BBP仅提供接口。
   UNION结构 ：无 */
#define BBPCOMM_MEAS_FLAG_GS_RD_ADDR                  (SOC_BBP_COMM_BASE_ADDR + 0x650)

/* 寄存器说明：寄存器
   详      述：L为从模时写此寄存器，W/TD/GM/GS主模读取此寄存器。此寄存器由DSP之间进行读写，BBP仅提供接口。
   UNION结构 ：无 */
#define BBPCOMM_WAKE_FLAG_L_WR_ADDR                   (SOC_BBP_COMM_BASE_ADDR + 0x654)

/* 寄存器说明：寄存器
   详      述：W为从模时写此寄存器，L/TD/GM/GS主模读取此寄存器。此寄存器由DSP之间进行读写，BBP仅提供接口。
   UNION结构 ：无 */
#define BBPCOMM_WAKE_FLAG_W_WR_ADDR                   (SOC_BBP_COMM_BASE_ADDR + 0x658)

/* 寄存器说明：寄存器
   详      述：TD为从模时写此寄存器，W/L/GM/GS主模读取此寄存器。此寄存器由DSP之间进行读写，BBP仅提供接口。
   UNION结构 ：无 */
#define BBPCOMM_WAKE_FLAG_T_WR_ADDR                   (SOC_BBP_COMM_BASE_ADDR + 0x65C)

/* 寄存器说明：寄存器
   详      述：GM为从模时写此寄存器，W/TD/L/GS主模读取此寄存器。此寄存器由DSP之间进行读写，BBP仅提供接口。
   UNION结构 ：无 */
#define BBPCOMM_WAKE_FLAG_GM_WR_ADDR                  (SOC_BBP_COMM_BASE_ADDR + 0x660)

/* 寄存器说明：寄存器
   详      述：GS为从模时写此寄存器，W/TD/GM/L主模读取此寄存器。此寄存器由DSP之间进行读写，BBP仅提供接口。
   UNION结构 ：无 */
...
/* 寄存器说明：寄存器
 bit[31:18]  保留
 bit[17:0]   TDS的系统定时。由DSP 配置timing_get_ind触发上报。本寄存器上报的是是TDS的framc帧计数
             上报时刻：timing_valid为高时上报有效
...
/* 寄存器说明：寄存器
 bit[31:1]   保留
 bit[0]      0:g2的SSI/MIPI控制字走SSI0/MIPI0
             1:g2的SSI/MIPI控制字走SSI1/MIPI1
             
             V7R2删除。
   UNION结构:  BBPCOMM_G2_CH_SWITCH_UNION */
#define BBPCOMM_G2_CH_SWITCH_ADDR                     (SOC_BBP_COMM_BASE_ADDR + 0x6B0)
...
/***======================================================================***
                     (2/4) register_define_irm_cs
 ***======================================================================***/
/* 寄存器说明：
 bit[31:2]   保留
 bit[1:0]    用于irm_cs模式选择，影响irm_ps内部时钟切换和模式切换，默认都为0。.
             2'b01：UMTS模式（包括均衡，译码R99相关RAM）
             2'b10：LTE模式（上行HARQ部分）
             2'b00：无（default）
...
/***======================================================================***
                     (3/4) register_define_irm_ps
 ***======================================================================***/
/* 寄存器说明：
 bit[31:5]   保留
 bit[4:0]    用于irm_ps网络模式选择，影响irm_ps内部时钟切换和模式切换，默认都为0。.
             5'b00001：GBBP1模式
             5'b00110：UMTS模式
             5'b01000：LTE模式
             5'b10000：TDS-CDMA模式
...
/*****************************************************************************
 结构名    : BBPCOMM_C0_CH_NUM_IND_UNION
 结构说明  : C0_CH_NUM_IND 寄存器结构定义。地址偏移量:0x0008，初值:0x00000000，宽度:32
 寄存器说明: 从模0对应通道的指示寄存器
*****************************************************************************/
typedef union
{
    unsigned long      c0_ch_num_ind_reg;
    struct
    {
        unsigned long  c0_ch_num_ind : 1;  /* bit[0-0] : 从模0对应的通道选择信号。
                                                         0：选择通道0；
                                                         1：选择通道1。
                                                         配置时刻：配置时刻：各模根据通信场景配置
...
} BBPCOMM_C0_CH_NUM_IND_UNION;
#define BBPCOMM_C0_CH_NUM_IND_c0_ch_num_ind_START  (0)
#define BBPCOMM_C0_CH_NUM_IND_c0_ch_num_ind_END    (0)
...
/*****************************************************************************
 ...
 结构说明  : CPU_MIPI0_CLR_IND_IMI 寄存器结构定义。地址偏移量:0x0114，初值:0x00000000，宽度:32
 寄存器说明: 
*****************************************************************************/
typedef union
{
    unsigned long      cpu_mipi0_clr_ind_imi_reg;
    struct
    {
        unsigned long  cpu_mipi0_clr_ind_imi : 1;  /* bit[0]   : cpu配置mipi功能,mipi0master清异常信号 */
        unsigned long  reserved              : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned long  cpu_rx0_lte_ckg_bypass : 1;  /* bit[0]    :  */
        unsigned long  cpu_rx0_tds_ckg_bypass : 1;  /* bit[1]    :  */
        unsigned long  cpu_rx0_w_ckg_bypass   : 1;  /* bit[2]    :  */
        unsigned long  cpu_rx0_g1_ckg_bypass  : 1;  /* bit[3]    :  */
        unsigned long  cpu_rx0_g2_ckg_bypass  : 1;  /* bit[4]    :  */
        unsigned long  cpu_tx0_lte_ckg_bypass : 1;  /* bit[5]    :  */
        unsigned long  cpu_tx0_tds_ckg_bypass : 1;  /* bit[6]    :  */
        unsigned long  cpu_tx0_w_ckg_bypass   : 1;  /* bit[7]    :  */
        unsigned long  cpu_tx0_g1_ckg_bypass  : 1;  /* bit[8]    :  */
        unsigned long  cpu_tx0_g2_ckg_bypass  : 1;  /* bit[9]    :  */
...
/*****************************************************************************
 结构名    : BBPCOMM_C1_CH_NUM_IND_UNION
 结构说明  : C1_CH_NUM_IND 寄存器结构定义。地址偏移量:0x0308，初值:0x00000001，宽度:32
 寄存器说明: 从模1对应通道的指示寄存器
*****************************************************************************/
typedef union
{
    unsigned long      c1_ch_num_ind_reg;
    struct
    {
        unsigned long  c1_ch_num_ind : 1;  /* bit[0-0] : 从模1对应的通道选择信号。
                                                         0：选择通道0；
                                                         1：选择通道1。
                                                         配置时刻：各模根据通信场景配置
...
} BBPCOMM_C1_CH_NUM_IND_UNION;
#define BBPCOMM_C1_CH_NUM_IND_c1_ch_num_ind_START  (0)
#define BBPCOMM_C1_CH_NUM_IND_c1_ch_num_ind_END    (0)
...
typedef union
{
    ...
    struct
    {
        unsigned long  dsp_mipi1_en_imi : 1;  /* bit[0]   : 由软件直接控制MIPI1 master,需首先拉高此指示,屏蔽xbbp对MIPI0的控制 */
        unsigned long  reserved         : 31; /* bit[1-31]: 保留 */
    } reg;
...
/*****************************************************************************
 ...
 结构说明  : CPU_MIPI1_CLR_IND_IMI 寄存器结构定义。地址偏移量:0x0414，初值:0x00000000，宽度:32
 寄存器说明: 
*****************************************************************************/
typedef union
{
    unsigned long      cpu_mipi1_clr_ind_imi_reg;
    struct
    {
        unsigned long  cpu_mipi1_clr_ind_imi : 1;  /* bit[0]   : cpu配置mipi功能,mipi1master清异常信号 */
        unsigned long  reserved              : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned long  cpu_rx1_lte_ckg_bypass : 1;  /* bit[0]    :  */
        unsigned long  cpu_rx1_tds_ckg_bypass : 1;  /* bit[1]    :  */
        unsigned long  cpu_rx1_w_ckg_bypass   : 1;  /* bit[2]    :  */
        unsigned long  cpu_rx1_g1_ckg_bypass  : 1;  /* bit[3]    :  */
        unsigned long  cpu_rx1_g2_ckg_bypass  : 1;  /* bit[4]    :  */
        unsigned long  cpu_tx1_lte_ckg_bypass : 1;  /* bit[5]    :  */
        unsigned long  cpu_tx1_tds_ckg_bypass : 1;  /* bit[6]    :  */
        unsigned long  cpu_tx1_w_ckg_bypass   : 1;  /* bit[7]    :  */
        unsigned long  cpu_tx1_g1_ckg_bypass  : 1;  /* bit[8]    :  */
        unsigned long  cpu_tx1_g2_ckg_bypass  : 1;  /* bit[9]    :  */
        unsigned long  reserved               : 22; /* bit[10-31]: 保留 */
    } reg;
...
/*****************************************************************************
 结构名    : BBPCOMM_L_CH_SWITCH_UNION
 结构说明  : L_CH_SWITCH 寄存器结构定义。地址偏移量:0x0600，初值:0x00000000，宽度:32
 寄存器说明: 
*****************************************************************************/
typedef union
{
    unsigned long      l_ch_switch_reg;
    struct
    {
        unsigned long  l_ch_switch : 1;  /* bit[0]   : 0:lte的SSI/MIPI控制字走SSI0/MIPI0
                                                       1:lte的SSI/MIPI控制字走SSI1/MIPI1
                                                       
                                                       V7R2删除。 */
        unsigned long  reserved    : 31; /* bit[1-31]: 保留 */
    } reg;
} BBPCOMM_L_CH_SWITCH_UNION;
#define BBPCOMM_L_CH_SWITCH_l_ch_switch_START  (0)
#define BBPCOMM_L_CH_SWITCH_l_ch_switch_END    (0)


/*****************************************************************************
 结构名    : BBPCOMM_W_CH_SWITCH_UNION
 结构说明  : W_CH_SWITCH 寄存器结构定义。地址偏移量:0x0604，初值:0x00000000，宽度:32
 寄存器说明: 
*****************************************************************************/
typedef union
{
    unsigned long      w_ch_switch_reg;
    struct
    {
        unsigned long  w_ch_switch : 1;  /* bit[0]   : 0:w的SSI/MIPI控制字走SSI0/MIPI0
                                                       1:w的SSI/MIPI控制字走SSI1/MIPI1
                                                       
                                                       V7R2删除。 */
        unsigned long  reserved    : 31; /* bit[1-31]: 保留 */
    } reg;
} BBPCOMM_W_CH_SWITCH_UNION;
#define BBPCOMM_W_CH_SWITCH_w_ch_switch_START  (0)
#define BBPCOMM_W_CH_SWITCH_w_ch_switch_END    (0)


/*****************************************************************************
 结构名    : BBPCOMM_T_CH_SWITCH_UNION
 结构说明  : T_CH_SWITCH 寄存器结构定义。地址偏移量:0x0608，初值:0x00000000，宽度:32
 寄存器说明: 
*****************************************************************************/
typedef union
{
    unsigned long      t_ch_switch_reg;
    struct
    {
        unsigned long  t_ch_switch : 1;  /* bit[0]   : 0:tds的SSI/MIPI控制字走SSI0/MIPI0
                                                       1:tds的SSI/MIPI控制字走SSI1/MIPI1
                                                       
                                                       V7R2删除。 */
        unsigned long  reserved    : 31; /* bit[1-31]: 保留 */
    } reg;
} BBPCOMM_T_CH_SWITCH_UNION;
#define BBPCOMM_T_CH_SWITCH_t_ch_switch_START  (0)
#define BBPCOMM_T_CH_SWITCH_t_ch_switch_END    (0)


/*****************************************************************************
 结构名    : BBPCOMM_G1_CH_SWITCH_UNION
 结构说明  : G1_CH_SWITCH 寄存器结构定义。地址偏移量:0x060C，初值:0x00000000，宽度:32
 寄存器说明: 
*****************************************************************************/
typedef union
{
    unsigned long      g1_ch_switch_reg;
    struct
    {
        unsigned long  g1_ch_switch : 1;  /* bit[0]   : 0:g1的SSI/MIPI控制字走SSI0/MIPI0
                                                        1:g1的SSI/MIPI控制字走SSI1/MIPI1
                                                        
                                                        V7R2删除。 */
        unsigned long  reserved     : 31; /* bit[1-31]: 保留 */
    } reg;
} BBPCOMM_G1_CH_SWITCH_UNION;
#define BBPCOMM_G1_CH_SWITCH_g1_ch_switch_START  (0)
#define BBPCOMM_G1_CH_SWITCH_g1_ch_switch_END    (0)


/*****************************************************************************
 结构名    : BBPCOMM_TIMING_GET_IND_UNION
 结构说明  : TIMING_GET_IND 寄存器结构定义。地址偏移量:0x0610，初值:0x00000000，宽度:32
 寄存器说明: 定时锁存使能指示寄存器
*****************************************************************************/
typedef union
{
    ...
    struct
    {
        unsigned long  timing_get_ind : 1;  /* bit[0]   : 逻辑自清零。DSP需要获取定时关系时配置生效，CTU逻辑根据此信号锁存LTE/W/Gm/Gs/TDS的系统定时，CTU的内部定时供DSP查询。
                                                          配置时刻：软件需要锁存定时
                                                          生效时刻：立即生效 */
        unsigned long  reserved       : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned long  timing_clear : 1;  /* bit[0]   : 逻辑自清零。定时标志清除，用于清除timing_valid标志。
                                                        配置时刻：DSP读取定时完毕后配置
                                                        生效时刻：立即生效 */
        unsigned long  reserved     : 31; /* bit[1-31]: 保留 */
    } reg;
...
/*****************************************************************************
 ...
 结构说明  : TIMING_VALID 寄存器结构定义。地址偏移量:0x0668，初值:0x00000000，宽度:32
 寄存器说明: 寄存器
*****************************************************************************/
typedef union
{
...
typedef union
{
    ...
    struct
    {
        unsigned long  o_tds_timing2_dsp : 18; /* bit[0-17] : TDS的系统定时。由DSP 配置timing_get_ind触发上报。本寄存器上报的是是TDS的framc帧计数
                                                              上报时刻：timing_valid为高时上报有效 */
...
#define BBPCOMM_TDS_TIMING2_DSP_o_tds_timing2_dsp_END    (17)
...
/*****************************************************************************
 结构名    : BBPCOMM_G2_CH_SWITCH_UNION
 结构说明  : G2_CH_SWITCH 寄存器结构定义。地址偏移量:0x06B0，初值:0x00000001，宽度:32
 寄存器说明: 寄存器
*****************************************************************************/
typedef union
{
    unsigned long      g2_ch_switch_reg;
    struct
    {
        unsigned long  g2_ch_switch : 1;  /* bit[0]   : 0:g2的SSI/MIPI控制字走SSI0/MIPI0
                                                        1:g2的SSI/MIPI控制字走SSI1/MIPI1
                                                        
                                                        V7R2删除。 */
        unsigned long  reserved     : 31; /* bit[1-31]: 保留 */
    } reg;
} BBPCOMM_G2_CH_SWITCH_UNION;
#define BBPCOMM_G2_CH_SWITCH_g2_ch_switch_START  (0)
#define BBPCOMM_G2_CH_SWITCH_g2_ch_switch_END    (0)
...
typedef union
{
    ...
    struct
    {
        ...
        unsigned long  reserved_3     : 7;  /* bit[9-15] : 保留 */
...
typedef union
{
    ...
    struct
    {
        unsigned long  irm_cs_ram_ctrl : 2;  /* bit[0-1] : 用于irm_cs模式选择，影响irm_ps内部时钟切换和模式切换，默认都为0。.
                                                           2'b01：UMTS模式（包括均衡，译码R99相关RAM）
                                                           2'b10：LTE模式（上行HARQ部分）
                                                           2'b00：无（default） */
        unsigned long  reserved        : 30; /* bit[2-31]: 保留 */
...
#define BBPCOMM_IRM_CS_RAM_CTRL_irm_cs_ram_ctrl_END    (1)
...
typedef union
{
    ...
    struct
    {
        unsigned long  irm_ps_ram_ctrl : 5;  /* bit[0-4] : 用于irm_ps网络模式选择，影响irm_ps内部时钟切换和模式切换，默认都为0。.
                                                           5'b00001：GBBP1模式
                                                           5'b00110：UMTS模式
                                                           5'b01000：LTE模式
                                                           5'b10000：TDS-CDMA模式
...
#define BBPCOMM_IRM_PS_RAM_CTRL_irm_ps_ram_ctrl_END    (4)
